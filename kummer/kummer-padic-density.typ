...
the flat one is about to die at level 2. This section applies it to the sweep of @tab-primes and
@sec-cert-extended, where the flat one works perfectly well --- which is exactly why it is the
right place to try the rewrite. `cert-extended.gp` is untouched; `cert-lprimary.gp` runs beside it,
and has to reproduce its verdicts.

*The decomposition is sharper here than for the ledger.* Write
$ cal(G) = E^d (QQ_p) = cal(G)_p ⊕ plus.big_(ell != p) cal(G)_ell , quad
  cal(G)_p = ZZ_p times T[p^infinity], quad cal(G)_ell = T[ell^infinity] . $
A closed subgroup decomposes the same way, so $[cal(G) : R] = product_ell [cal(G)_ell : R_ell]$ and
$ E^d (QQ) " dense in " E^d (QQ_p) quad <==> quad R_ell = cal(G)_ell " for every " ell . $
For the ledger, coverage did *not* split like this: a pair must lie in *one and the same* entry, so
the choice of entry is coupled across layers and the test became a mask intersection. Density of a
*single* reach has no such coupling --- these are genuinely independent conditions, one per layer.

*The two kinds of layer, as in @sec-lprimary.* The diagonal $ell = p$ is $ZZ_p$, since
@sec-cert-ppart shows $T[p^infinity] = 0$ on every line here, so $R_p = p^k ZZ_p$ and the layer
index is $p^k$ --- one integer, exact. And $k$ needs no search: $"depth" (M P) = v_p (alpha) + 1$
gives $k = min_i "depth" (M P_i) - 1$, valid at the additive places too, where $v_p (M) > 0$ makes
the $(m, v)$ bookkeeping of @sec-cert-conv fiddly. Every off-diagonal layer is finite of order
$ell^(v_ell (M))$ and, since $E_1$ is pro-$p$, injects into $cal(G) slash E_1$ --- so the whole
computation happens in a group of order $M$: multiply each image by $M slash ell^(v_ell (M))$ to
land in the $ell$-part, close under addition, compare orders.

#block(fill: rgb("#eef4ff"), inset: 9pt, radius: 3pt, width: 100%)[
  *No discrete logarithms anywhere*, which is the incidental prize and not a small one. @sec-cert-ledger
  and @sec-cert-extended both record the same trap: `ellgroup(E,p,1)` returns a generator chosen at
  random on each call, so a class column computed against it is irreproducible, and two generators
  logged against separate calls are not even mutually consistent. A *layer index is an order, not a
  coordinate*. It needs no base, so the trap cannot arise. Cosets are compared by `inE1` on the
  difference, so no coset table is built either --- `arenainit`'s is $O(M^3)$ and hopeless at
  $p = 199$.
]

*It reproduces @tab-primes exactly*: all $180$ blocks come out with total index $1$, no
disagreements. What it adds is that the index is now *itemised*. A flat index of $6$ does not say
whether the obstruction is at $2$, at $3$, or one of each; and on the lines needing two generators
there is no single $m$ to report at all, so the flat index has to come from a lattice determinant.

*And the itemisation immediately corrects something.* @sec-cert-extended found $22$ lines where no
single generator is dense and filed them all under "$E^d (QQ_p)$ is not procyclic". Those are two
different statements, and the layer view separates them: $cal(G)$ is procyclic exactly when every
layer is cyclic, i.e. when no $cal(G)_ell$ has rank $2$.

#align(center)[
#table(columns: 2, align: (right, left), stroke: 0.4pt, inset: 6pt,
  [$21$ lines], [some layer has rank $2$, so $cal(G)$ is *not* procyclic: no single point of
                 $E^d (QQ_p)$ generates and a rank-$1$ twist could never have worked],
  [$1$ line],   [$cal(G)$ *is* procyclic --- neither point the search returned is dense, but
                 another point of the same group is],
)]

The exception is $p = 83$, class $[u]$, $d = -11$, and the decomposition says exactly what goes
wrong. There $tilde(E)(bb(F)_83) tilde.equiv C_78$ is cyclic, so
$E^d (QQ_83) tilde.equiv ZZ_83 times C_78$ is procyclic. Taken alone, $P_1$ has total index $3$ and
$P_2$ has total index $13$ --- *they fail at different layers*, which is precisely what the flat
index hides and the decomposition exposes. Together they give index $1$; and so does the single
point $-P_1 - 2 P_2$, among others. One generator does suffice at $p = 83$. The basis
`ellsaturation` returned simply missed it.

=== The table <tab-lprimary>

One line per $(p, d, ell)$, following the shape of @tab-ledger-extended's one line per (twist,
generator). Reading it:

- $ell$ marked #super[d] is the *diagonal* layer $ell = p$, the only infinite one. There
  $cal(G)_ell = ZZ_p$ and $R_ell = p^k ZZ_p$, so the index is $p^k$ and the entry $1$ says
  $k = 0$.
- Every other $ell$ is a finite layer $T[ell^infinity]$ of order $ell^(v_ell (M))$.
- *rank* is the minimal number of generators of $cal(G)_ell$. A #super[\*] marks rank $2$ ---
  a non-cyclic layer, which is what forces rank $2$ on that line, and there are $21$ of them.
- *index* is $[cal(G)_ell : R_ell]$. Index $1$ says $R_ell = cal(G)_ell$: the layer is full.

Density at $p$ is the statement that *every* index in the last column is $1$ --- which it is, on
all $419$ lines, reproducing @tab-primes with no disagreement.

#set text(size: 7.2pt)
#table(
  columns: 7,
  align: (right, center, right, right, center, center, right),
  stroke: 0.35pt + luma(175),
  inset: (x: 4pt, y: 2.5pt),
  table.header([$p$], [class], [$d$], [$ell$], [$cal(G)_ell$], [rank], [index]),
  [$3$], [$[1]$], [$7$], [$3$#super[d]], [$ZZ_(3)$], [$1$], [$1$],
  [$3$], [$[1]$], [$7$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$3$], [$[u]$], [$-1$], [$3$#super[d]], [$ZZ_(3)$], [$1$], [$1$],
  [$3$], [$[u]$], [$-1$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$3$], [$[p]$], [$3$], [$3$#super[d]], [$ZZ_(3)$], [$1$], [$1$],
  [$3$], [$[p]$], [$3$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$3$], [$[u p]$], [$6$], [$3$#super[d]], [$ZZ_(3)$], [$1$], [$1$],
  [$3$], [$[u p]$], [$6$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$5$], [$[1]$], [$1$], [$5$#super[d]], [$ZZ_(5)$], [$1$], [$1$],
  [$5$], [$[1]$], [$1$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$5$], [$[u]$], [$3$], [$5$#super[d]], [$ZZ_(5)$], [$1$], [$1$],
  [$5$], [$[u]$], [$3$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$5$], [$[p]$], [$5$], [$5$#super[d]], [$ZZ_(5)$], [$1$], [$1$],
  [$5$], [$[u p]$], [$-35$], [$5$#super[d]], [$ZZ_(5)$], [$1$], [$1$],
  [$7$], [$[1]$], [$1$], [$7$#super[d]], [$ZZ_(7)$], [$1$], [$1$],
  [$7$], [$[1]$], [$1$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$7$], [$[u]$], [$-1$], [$7$#super[d]], [$ZZ_(7)$], [$1$], [$1$],
  [$7$], [$[u]$], [$-1$], [$11$], [$C_(11)$], [$1$], [$1$],
  [$7$], [$[p]$], [$7$], [$7$#super[d]], [$ZZ_(7)$], [$1$], [$1$],
  [$7$], [$[u p]$], [$-7$], [$7$#super[d]], [$ZZ_(7)$], [$1$], [$1$],
  [$11$], [$[1]$], [$3$], [$11$#super[d]], [$ZZ_(11)$], [$1$], [$1$],
  [$11$], [$[1]$], [$3$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$11$], [$[1]$], [$3$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$11$], [$[u]$], [$6$], [$11$#super[d]], [$ZZ_(11)$], [$1$], [$1$],
  [$11$], [$[u]$], [$6$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$11$], [$[u]$], [$6$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$11$], [$[p]$], [$11$], [$11$#super[d]], [$ZZ_(11)$], [$1$], [$1$],
  [$11$], [$[p]$], [$11$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$11$], [$[u p]$], [$-11$], [$11$#super[d]], [$ZZ_(11)$], [$1$], [$1$],
  [$11$], [$[u p]$], [$-11$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$13$], [$[1]$], [$-1$], [$13$#super[d]], [$ZZ_(13)$], [$1$], [$1$],
  [$13$], [$[1]$], [$-1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$13$], [$[1]$], [$-1$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$13$], [$[u]$], [$5$], [$13$#super[d]], [$ZZ_(13)$], [$1$], [$1$],
  [$13$], [$[u]$], [$5$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$13$], [$[u]$], [$5$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$13$], [$[p]$], [$-13$], [$13$#super[d]], [$ZZ_(13)$], [$1$], [$1$],
  [$13$], [$[p]$], [$-13$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$13$], [$[u p]$], [$26$], [$13$#super[d]], [$ZZ_(13)$], [$1$], [$1$],
  [$13$], [$[u p]$], [$26$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$17$], [$[1]$], [$-1$], [$17$#super[d]], [$ZZ_(17)$], [$1$], [$1$],
  [$17$], [$[1]$], [$-1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$17$], [$[1]$], [$-1$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$17$], [$[u]$], [$7$], [$17$#super[d]], [$ZZ_(17)$], [$1$], [$1$],
  [$17$], [$[u]$], [$7$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$17$], [$[u]$], [$7$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$17$], [$[p]$], [$34$], [$17$#super[d]], [$ZZ_(17)$], [$1$], [$1$],
  [$17$], [$[p]$], [$34$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$17$], [$[u p]$], [$51$], [$17$#super[d]], [$ZZ_(17)$], [$1$], [$1$],
  [$17$], [$[u p]$], [$51$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$19$], [$[1]$], [$1$], [$19$#super[d]], [$ZZ_(19)$], [$1$], [$1$],
  [$19$], [$[1]$], [$1$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$19$], [$[1]$], [$1$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$19$], [$[u]$], [$-1$], [$19$#super[d]], [$ZZ_(19)$], [$1$], [$1$],
  [$19$], [$[p]$], [$95$], [$19$#super[d]], [$ZZ_(19)$], [$1$], [$1$],
  [$19$], [$[u p]$], [$-95$], [$19$#super[d]], [$ZZ_(19)$], [$1$], [$1$],
  [$23$], [$[1]$], [$1$], [$23$#super[d]], [$ZZ_(23)$], [$1$], [$1$],
  [$23$], [$[1]$], [$1$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$23$], [$[1]$], [$1$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$23$], [$[u]$], [$-1$], [$23$#super[d]], [$ZZ_(23)$], [$1$], [$1$],
  [$23$], [$[u]$], [$-1$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$23$], [$[u]$], [$-1$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$23$], [$[p]$], [$46$], [$23$#super[d]], [$ZZ_(23)$], [$1$], [$1$],
  [$23$], [$[p]$], [$46$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$23$], [$[u p]$], [$115$], [$23$#super[d]], [$ZZ_(23)$], [$1$], [$1$],
  [$23$], [$[u p]$], [$115$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$29$], [$[1]$], [$-1$], [$29$#super[d]], [$ZZ_(29)$], [$1$], [$1$],
  [$29$], [$[1]$], [$-1$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$29$], [$[1]$], [$-1$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$29$], [$[u]$], [$11$], [$29$#super[d]], [$ZZ_(29)$], [$1$], [$1$],
  [$29$], [$[u]$], [$11$], [$2$], [$C_(8)$], [$1$], [$1$],
  [$29$], [$[u]$], [$11$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$29$], [$[p]$], [$-29$], [$29$#super[d]], [$ZZ_(29)$], [$1$], [$1$],
  [$29$], [$[p]$], [$-29$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$29$], [$[u p]$], [$58$], [$29$#super[d]], [$ZZ_(29)$], [$1$], [$1$],
  [$29$], [$[u p]$], [$58$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$31$], [$[1]$], [$1$], [$31$#super[d]], [$ZZ_(31)$], [$1$], [$1$],
  [$31$], [$[1]$], [$1$], [$2$], [$C_(32)$], [$1$], [$1$],
  [$31$], [$[u]$], [$-1$], [$31$#super[d]], [$ZZ_(31)$], [$1$], [$1$],
  [$31$], [$[u]$], [$-1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$31$], [$[u]$], [$-1$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$31$], [$[u]$], [$-1$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$31$], [$[p]$], [$31$], [$31$#super[d]], [$ZZ_(31)$], [$1$], [$1$],
  [$31$], [$[p]$], [$31$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$31$], [$[u p]$], [$-62$], [$31$#super[d]], [$ZZ_(31)$], [$1$], [$1$],
  [$31$], [$[u p]$], [$-62$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$37$], [$[1]$], [$-11$], [$37$#super[d]], [$ZZ_(37)$], [$1$], [$1$],
  [$37$], [$[1]$], [$-11$], [$2$], [$C_(16)$], [$1$], [$1$],
  [$37$], [$[1]$], [$-11$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$37$], [$[u]$], [$6$], [$37$#super[d]], [$ZZ_(37)$], [$1$], [$1$],
  [$37$], [$[u]$], [$6$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$37$], [$[u]$], [$6$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$37$], [$[p]$], [$-37$], [$37$#super[d]], [$ZZ_(37)$], [$1$], [$1$],
  [$37$], [$[p]$], [$-37$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$37$], [$[u p]$], [$74$], [$37$#super[d]], [$ZZ_(37)$], [$1$], [$1$],
  [$37$], [$[u p]$], [$74$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$41$], [$[1]$], [$-1$], [$41$#super[d]], [$ZZ_(41)$], [$1$], [$1$],
  [$41$], [$[1]$], [$-1$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$41$], [$[1]$], [$-1$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$41$], [$[u]$], [$3$], [$41$#super[d]], [$ZZ_(41)$], [$1$], [$1$],
  [$41$], [$[u]$], [$3$], [$7$], [$C_(49)$], [$1$], [$1$],
  [$41$], [$[p]$], [$41$], [$41$#super[d]], [$ZZ_(41)$], [$1$], [$1$],
  [$41$], [$[u p]$], [$123$], [$41$#super[d]], [$ZZ_(41)$], [$1$], [$1$],
  [$43$], [$[1]$], [$1$], [$43$#super[d]], [$ZZ_(43)$], [$1$], [$1$],
  [$43$], [$[1]$], [$1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$43$], [$[1]$], [$1$], [$17$], [$C_(17)$], [$1$], [$1$],
  [$43$], [$[u]$], [$-1$], [$43$#super[d]], [$ZZ_(43)$], [$1$], [$1$],
  [$43$], [$[u]$], [$-1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$43$], [$[u]$], [$-1$], [$3$], [$C_(27)$], [$1$], [$1$],
  [$43$], [$[p]$], [$-86$], [$43$#super[d]], [$ZZ_(43)$], [$1$], [$1$],
  [$43$], [$[p]$], [$-86$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$43$], [$[u p]$], [$86$], [$43$#super[d]], [$ZZ_(43)$], [$1$], [$1$],
  [$43$], [$[u p]$], [$86$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$47$], [$[1]$], [$-11$], [$47$#super[d]], [$ZZ_(47)$], [$1$], [$1$],
  [$47$], [$[1]$], [$-11$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$47$], [$[1]$], [$-11$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$47$], [$[1]$], [$-11$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$47$], [$[u]$], [$-149$], [$47$#super[d]], [$ZZ_(47)$], [$1$], [$1$],
  [$47$], [$[u]$], [$-149$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$47$], [$[u]$], [$-149$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$47$], [$[p]$], [$94$], [$47$#super[d]], [$ZZ_(47)$], [$1$], [$1$],
  [$47$], [$[p]$], [$94$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$47$], [$[u p]$], [$705$], [$47$#super[d]], [$ZZ_(47)$], [$1$], [$1$],
  [$47$], [$[u p]$], [$705$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$53$], [$[1]$], [$11$], [$53$#super[d]], [$ZZ_(53)$], [$1$], [$1$],
  [$53$], [$[1]$], [$11$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$53$], [$[1]$], [$11$], [$29$], [$C_(29)$], [$1$], [$1$],
  [$53$], [$[u]$], [$22$], [$53$#super[d]], [$ZZ_(53)$], [$1$], [$1$],
  [$53$], [$[u]$], [$22$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$53$], [$[u]$], [$22$], [$5$], [$C_(25)$], [$1$], [$1$],
  [$53$], [$[p]$], [$53$], [$53$#super[d]], [$ZZ_(53)$], [$1$], [$1$],
  [$53$], [$[p]$], [$53$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$53$], [$[u p]$], [$106$], [$53$#super[d]], [$ZZ_(53)$], [$1$], [$1$],
  [$53$], [$[u p]$], [$106$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$59$], [$[1]$], [$1$], [$59$#super[d]], [$ZZ_(59)$], [$1$], [$1$],
  [$59$], [$[1]$], [$1$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$59$], [$[1]$], [$1$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$59$], [$[u]$], [$6$], [$59$#super[d]], [$ZZ_(59)$], [$1$], [$1$],
  [$59$], [$[u]$], [$6$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$59$], [$[u]$], [$6$], [$19$], [$C_(19)$], [$1$], [$1$],
  [$59$], [$[p]$], [$295$], [$59$#super[d]], [$ZZ_(59)$], [$1$], [$1$],
  [$59$], [$[u p]$], [$-59$], [$59$#super[d]], [$ZZ_(59)$], [$1$], [$1$],
  [$61$], [$[1]$], [$1$], [$61$#super[d]], [$ZZ_(61)$], [$1$], [$1$],
  [$61$], [$[1]$], [$1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$61$], [$[1]$], [$1$], [$5$], [$C_(25)$], [$1$], [$1$],
  [$61$], [$[u]$], [$7$], [$61$#super[d]], [$ZZ_(61)$], [$1$], [$1$],
  [$61$], [$[u]$], [$7$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$61$], [$[u]$], [$7$], [$37$], [$C_(37)$], [$1$], [$1$],
  [$61$], [$[p]$], [$-61$], [$61$#super[d]], [$ZZ_(61)$], [$1$], [$1$],
  [$61$], [$[p]$], [$-61$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$61$], [$[u p]$], [$122$], [$61$#super[d]], [$ZZ_(61)$], [$1$], [$1$],
  [$61$], [$[u p]$], [$122$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$67$], [$[1]$], [$-221$], [$67$#super[d]], [$ZZ_(67)$], [$1$], [$1$],
  [$67$], [$[1]$], [$-221$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$67$], [$[1]$], [$-221$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$67$], [$[u]$], [$51$], [$67$#super[d]], [$ZZ_(67)$], [$1$], [$1$],
  [$67$], [$[u]$], [$51$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$67$], [$[u]$], [$51$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$67$], [$[p]$], [$2211$], [$67$#super[d]], [$ZZ_(67)$], [$1$], [$1$],
  [$67$], [$[p]$], [$2211$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$67$], [$[u p]$], [$134$], [$67$#super[d]], [$ZZ_(67)$], [$1$], [$1$],
  [$67$], [$[u p]$], [$134$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$71$], [$[1]$], [$1$], [$71$#super[d]], [$ZZ_(71)$], [$1$], [$1$],
  [$71$], [$[1]$], [$1$], [$59$], [$C_(59)$], [$1$], [$1$],
  [$71$], [$[u]$], [$-1$], [$71$#super[d]], [$ZZ_(71)$], [$1$], [$1$],
  [$71$], [$[u]$], [$-1$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$71$], [$[u]$], [$-1$], [$17$], [$C_(17)$], [$1$], [$1$],
  [$71$], [$[p]$], [$71$], [$71$#super[d]], [$ZZ_(71)$], [$1$], [$1$],
  [$71$], [$[u p]$], [$-71$], [$71$#super[d]], [$ZZ_(71)$], [$1$], [$1$],
  [$73$], [$[1]$], [$3$], [$73$#super[d]], [$ZZ_(73)$], [$1$], [$1$],
  [$73$], [$[1]$], [$3$], [$2$], [$C_(8)$], [$1$], [$1$],
  [$73$], [$[1]$], [$3$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$73$], [$[u]$], [$-21$], [$73$#super[d]], [$ZZ_(73)$], [$1$], [$1$],
  [$73$], [$[u]$], [$-21$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$73$], [$[u]$], [$-21$], [$19$], [$C_(19)$], [$1$], [$1$],
  [$73$], [$[p]$], [$146$], [$73$#super[d]], [$ZZ_(73)$], [$1$], [$1$],
  [$73$], [$[p]$], [$146$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$73$], [$[u p]$], [$-365$], [$73$#super[d]], [$ZZ_(73)$], [$1$], [$1$],
  [$73$], [$[u p]$], [$-365$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$79$], [$[1]$], [$1$], [$79$#super[d]], [$ZZ_(79)$], [$1$], [$1$],
  [$79$], [$[1]$], [$1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$79$], [$[1]$], [$1$], [$43$], [$C_(43)$], [$1$], [$1$],
  [$79$], [$[u]$], [$-1$], [$79$#super[d]], [$ZZ_(79)$], [$1$], [$1$],
  [$79$], [$[u]$], [$-1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$79$], [$[u]$], [$-1$], [$37$], [$C_(37)$], [$1$], [$1$],
  [$79$], [$[p]$], [$158$], [$79$#super[d]], [$ZZ_(79)$], [$1$], [$1$],
  [$79$], [$[p]$], [$158$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$79$], [$[u p]$], [$-158$], [$79$#super[d]], [$ZZ_(79)$], [$1$], [$1$],
  [$79$], [$[u p]$], [$-158$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$83$], [$[1]$], [$-22$], [$83$#super[d]], [$ZZ_(83)$], [$1$], [$1$],
  [$83$], [$[1]$], [$-22$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$83$], [$[1]$], [$-22$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$83$], [$[1]$], [$-22$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$83$], [$[u]$], [$-11$], [$83$#super[d]], [$ZZ_(83)$], [$1$], [$1$],
  [$83$], [$[u]$], [$-11$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$83$], [$[u]$], [$-11$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$83$], [$[u]$], [$-11$], [$13$], [$C_(13)$], [$1$], [$1$],
  [$83$], [$[p]$], [$83$], [$83$#super[d]], [$ZZ_(83)$], [$1$], [$1$],
  [$83$], [$[p]$], [$83$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$83$], [$[u p]$], [$166$], [$83$#super[d]], [$ZZ_(83)$], [$1$], [$1$],
  [$83$], [$[u p]$], [$166$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$89$], [$[1]$], [$1$], [$89$#super[d]], [$ZZ_(89)$], [$1$], [$1$],
  [$89$], [$[1]$], [$1$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$89$], [$[1]$], [$1$], [$5$], [$C_(25)$], [$1$], [$1$],
  [$89$], [$[u]$], [$-7$], [$89$#super[d]], [$ZZ_(89)$], [$1$], [$1$],
  [$89$], [$[u]$], [$-7$], [$2$], [$C_(16)$], [$1$], [$1$],
  [$89$], [$[u]$], [$-7$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$89$], [$[p]$], [$178$], [$89$#super[d]], [$ZZ_(89)$], [$1$], [$1$],
  [$89$], [$[p]$], [$178$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$89$], [$[u p]$], [$-267$], [$89$#super[d]], [$ZZ_(89)$], [$1$], [$1$],
  [$89$], [$[u p]$], [$-267$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$97$], [$[1]$], [$1$], [$97$#super[d]], [$ZZ_(97)$], [$1$], [$1$],
  [$97$], [$[u]$], [$7$], [$97$#super[d]], [$ZZ_(97)$], [$1$], [$1$],
  [$97$], [$[u]$], [$7$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$97$], [$[u]$], [$7$], [$11$], [$C_(11)$], [$1$], [$1$],
  [$97$], [$[p]$], [$97$], [$97$#super[d]], [$ZZ_(97)$], [$1$], [$1$],
  [$97$], [$[u p]$], [$485$], [$97$#super[d]], [$ZZ_(97)$], [$1$], [$1$],
  [$101$], [$[1]$], [$-1$], [$101$#super[d]], [$ZZ_(101)$], [$1$], [$1$],
  [$101$], [$[1]$], [$-1$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$101$], [$[1]$], [$-1$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$101$], [$[1]$], [$-1$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$101$], [$[u]$], [$3$], [$101$#super[d]], [$ZZ_(101)$], [$1$], [$1$],
  [$101$], [$[u]$], [$3$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$101$], [$[u]$], [$3$], [$11$], [$C_(11)$], [$1$], [$1$],
  [$101$], [$[p]$], [$101$], [$101$#super[d]], [$ZZ_(101)$], [$1$], [$1$],
  [$101$], [$[u p]$], [$-2626$], [$101$#super[d]], [$ZZ_(101)$], [$1$], [$1$],
  [$103$], [$[1]$], [$1$], [$103$#super[d]], [$ZZ_(103)$], [$1$], [$1$],
  [$103$], [$[1]$], [$1$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$103$], [$[1]$], [$1$], [$29$], [$C_(29)$], [$1$], [$1$],
  [$103$], [$[u]$], [$-1$], [$103$#super[d]], [$ZZ_(103)$], [$1$], [$1$],
  [$103$], [$[u]$], [$-1$], [$11$], [$C_(121)$], [$1$], [$1$],
  [$103$], [$[p]$], [$103$], [$103$#super[d]], [$ZZ_(103)$], [$1$], [$1$],
  [$103$], [$[u p]$], [$2266$], [$103$#super[d]], [$ZZ_(103)$], [$1$], [$1$],
  [$107$], [$[1]$], [$-7$], [$107$#super[d]], [$ZZ_(107)$], [$1$], [$1$],
  [$107$], [$[1]$], [$-7$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$107$], [$[1]$], [$-7$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$107$], [$[1]$], [$-7$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$107$], [$[u]$], [$-1$], [$107$#super[d]], [$ZZ_(107)$], [$1$], [$1$],
  [$107$], [$[u]$], [$-1$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$107$], [$[u]$], [$-1$], [$37$], [$C_(37)$], [$1$], [$1$],
  [$107$], [$[p]$], [$-1605$], [$107$#super[d]], [$ZZ_(107)$], [$1$], [$1$],
  [$107$], [$[u p]$], [$-107$], [$107$#super[d]], [$ZZ_(107)$], [$1$], [$1$],
  [$109$], [$[1]$], [$1$], [$109$#super[d]], [$ZZ_(109)$], [$1$], [$1$],
  [$109$], [$[1]$], [$1$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$109$], [$[1]$], [$1$], [$41$], [$C_(41)$], [$1$], [$1$],
  [$109$], [$[u]$], [$6$], [$109$#super[d]], [$ZZ_(109)$], [$1$], [$1$],
  [$109$], [$[u]$], [$6$], [$97$], [$C_(97)$], [$1$], [$1$],
  [$109$], [$[p]$], [$109$], [$109$#super[d]], [$ZZ_(109)$], [$1$], [$1$],
  [$109$], [$[u p]$], [$654$], [$109$#super[d]], [$ZZ_(109)$], [$1$], [$1$],
  [$113$], [$[1]$], [$1$], [$113$#super[d]], [$ZZ_(113)$], [$1$], [$1$],
  [$113$], [$[1]$], [$1$], [$5$], [$C_(125)$], [$1$], [$1$],
  [$113$], [$[u]$], [$3$], [$113$#super[d]], [$ZZ_(113)$], [$1$], [$1$],
  [$113$], [$[u]$], [$3$], [$103$], [$C_(103)$], [$1$], [$1$],
  [$113$], [$[p]$], [$113$], [$113$#super[d]], [$ZZ_(113)$], [$1$], [$1$],
  [$113$], [$[u p]$], [$339$], [$113$#super[d]], [$ZZ_(113)$], [$1$], [$1$],
  [$127$], [$[1]$], [$-6$], [$127$#super[d]], [$ZZ_(127)$], [$1$], [$1$],
  [$127$], [$[1]$], [$-6$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$127$], [$[1]$], [$-6$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$127$], [$[1]$], [$-6$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$127$], [$[u]$], [$3$], [$127$#super[d]], [$ZZ_(127)$], [$1$], [$1$],
  [$127$], [$[u]$], [$3$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$127$], [$[u]$], [$3$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$127$], [$[u]$], [$3$], [$13$], [$C_(13)$], [$1$], [$1$],
  [$127$], [$[p]$], [$254$], [$127$#super[d]], [$ZZ_(127)$], [$1$], [$1$],
  [$127$], [$[p]$], [$254$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$127$], [$[u p]$], [$-127$], [$127$#super[d]], [$ZZ_(127)$], [$1$], [$1$],
  [$127$], [$[u p]$], [$-127$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$131$], [$[1]$], [$53$], [$131$#super[d]], [$ZZ_(131)$], [$1$], [$1$],
  [$131$], [$[1]$], [$53$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$131$], [$[u]$], [$-11$], [$131$#super[d]], [$ZZ_(131)$], [$1$], [$1$],
  [$131$], [$[u]$], [$-11$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$131$], [$[u]$], [$-11$], [$17$], [$C_(17)$], [$1$], [$1$],
  [$131$], [$[p]$], [$131$], [$131$#super[d]], [$ZZ_(131)$], [$1$], [$1$],
  [$131$], [$[p]$], [$131$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$131$], [$[u p]$], [$8646$], [$131$#super[d]], [$ZZ_(131)$], [$1$], [$1$],
  [$131$], [$[u p]$], [$8646$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$137$], [$[1]$], [$7$], [$137$#super[d]], [$ZZ_(137)$], [$1$], [$1$],
  [$137$], [$[1]$], [$7$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$137$], [$[1]$], [$7$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$137$], [$[1]$], [$7$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$137$], [$[u]$], [$3$], [$137$#super[d]], [$ZZ_(137)$], [$1$], [$1$],
  [$137$], [$[u]$], [$3$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$137$], [$[u]$], [$3$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$137$], [$[u]$], [$3$], [$5$], [$C_(25)$], [$1$], [$1$],
  [$137$], [$[p]$], [$274$], [$137$#super[d]], [$ZZ_(137)$], [$1$], [$1$],
  [$137$], [$[p]$], [$274$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$137$], [$[u p]$], [$-411$], [$137$#super[d]], [$ZZ_(137)$], [$1$], [$1$],
  [$137$], [$[u p]$], [$-411$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$139$], [$[1]$], [$51$], [$139$#super[d]], [$ZZ_(139)$], [$1$], [$1$],
  [$139$], [$[1]$], [$51$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$139$], [$[1]$], [$51$], [$3$], [$(ZZ slash 3)^2$], [$2$#super[\*]], [$1$],
  [$139$], [$[1]$], [$51$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$139$], [$[u]$], [$3$], [$139$#super[d]], [$ZZ_(139)$], [$1$], [$1$],
  [$139$], [$[u]$], [$3$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$139$], [$[u]$], [$3$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$139$], [$[u]$], [$3$], [$11$], [$C_(11)$], [$1$], [$1$],
  [$139$], [$[p]$], [$139$], [$139$#super[d]], [$ZZ_(139)$], [$1$], [$1$],
  [$139$], [$[p]$], [$139$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$139$], [$[u p]$], [$-139$], [$139$#super[d]], [$ZZ_(139)$], [$1$], [$1$],
  [$139$], [$[u p]$], [$-139$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$149$], [$[1]$], [$53$], [$149$#super[d]], [$ZZ_(149)$], [$1$], [$1$],
  [$149$], [$[1]$], [$53$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$149$], [$[1]$], [$53$], [$17$], [$C_(17)$], [$1$], [$1$],
  [$149$], [$[u]$], [$94$], [$149$#super[d]], [$ZZ_(149)$], [$1$], [$1$],
  [$149$], [$[u]$], [$94$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$149$], [$[u]$], [$94$], [$41$], [$C_(41)$], [$1$], [$1$],
  [$149$], [$[p]$], [$-149$], [$149$#super[d]], [$ZZ_(149)$], [$1$], [$1$],
  [$149$], [$[p]$], [$-149$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$149$], [$[u p]$], [$13559$], [$149$#super[d]], [$ZZ_(149)$], [$1$], [$1$],
  [$149$], [$[u p]$], [$13559$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$151$], [$[1]$], [$1$], [$151$#super[d]], [$ZZ_(151)$], [$1$], [$1$],
  [$151$], [$[1]$], [$1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$151$], [$[1]$], [$1$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$151$], [$[1]$], [$1$], [$11$], [$C_(11)$], [$1$], [$1$],
  [$151$], [$[u]$], [$-1$], [$151$#super[d]], [$ZZ_(151)$], [$1$], [$1$],
  [$151$], [$[u]$], [$-1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$151$], [$[u]$], [$-1$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$151$], [$[u]$], [$-1$], [$5$], [$C_(25)$], [$1$], [$1$],
  [$151$], [$[p]$], [$755$], [$151$#super[d]], [$ZZ_(151)$], [$1$], [$1$],
  [$151$], [$[p]$], [$755$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$151$], [$[u p]$], [$453$], [$151$#super[d]], [$ZZ_(151)$], [$1$], [$1$],
  [$151$], [$[u p]$], [$453$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$157$], [$[1]$], [$1$], [$157$#super[d]], [$ZZ_(157)$], [$1$], [$1$],
  [$157$], [$[1]$], [$1$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$157$], [$[1]$], [$1$], [$19$], [$C_(19)$], [$1$], [$1$],
  [$157$], [$[u]$], [$5$], [$157$#super[d]], [$ZZ_(157)$], [$1$], [$1$],
  [$157$], [$[u]$], [$5$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$157$], [$[u]$], [$5$], [$29$], [$C_(29)$], [$1$], [$1$],
  [$157$], [$[p]$], [$157$], [$157$#super[d]], [$ZZ_(157)$], [$1$], [$1$],
  [$157$], [$[u p]$], [$2355$], [$157$#super[d]], [$ZZ_(157)$], [$1$], [$1$],
  [$163$], [$[1]$], [$1$], [$163$#super[d]], [$ZZ_(163)$], [$1$], [$1$],
  [$163$], [$[1]$], [$1$], [$3$], [$C_(27)$], [$1$], [$1$],
  [$163$], [$[1]$], [$1$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$163$], [$[u]$], [$-1$], [$163$#super[d]], [$ZZ_(163)$], [$1$], [$1$],
  [$163$], [$[u]$], [$-1$], [$139$], [$C_(139)$], [$1$], [$1$],
  [$163$], [$[p]$], [$978$], [$163$#super[d]], [$ZZ_(163)$], [$1$], [$1$],
  [$163$], [$[u p]$], [$815$], [$163$#super[d]], [$ZZ_(163)$], [$1$], [$1$],
  [$167$], [$[1]$], [$-13$], [$167$#super[d]], [$ZZ_(167)$], [$1$], [$1$],
  [$167$], [$[1]$], [$-13$], [$2$], [$C_(16)$], [$1$], [$1$],
  [$167$], [$[1]$], [$-13$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$167$], [$[u]$], [$-6$], [$167$#super[d]], [$ZZ_(167)$], [$1$], [$1$],
  [$167$], [$[u]$], [$-6$], [$2$], [$C_(64)$], [$1$], [$1$],
  [$167$], [$[u]$], [$-6$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$167$], [$[p]$], [$334$], [$167$#super[d]], [$ZZ_(167)$], [$1$], [$1$],
  [$167$], [$[p]$], [$334$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$167$], [$[u p]$], [$-334$], [$167$#super[d]], [$ZZ_(167)$], [$1$], [$1$],
  [$167$], [$[u p]$], [$-334$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$173$], [$[1]$], [$51$], [$173$#super[d]], [$ZZ_(173)$], [$1$], [$1$],
  [$173$], [$[1]$], [$51$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$173$], [$[1]$], [$51$], [$43$], [$C_(43)$], [$1$], [$1$],
  [$173$], [$[u]$], [$53$], [$173$#super[d]], [$ZZ_(173)$], [$1$], [$1$],
  [$173$], [$[u]$], [$53$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$173$], [$[u]$], [$53$], [$11$], [$C_(11)$], [$1$], [$1$],
  [$173$], [$[p]$], [$-173$], [$173$#super[d]], [$ZZ_(173)$], [$1$], [$1$],
  [$173$], [$[p]$], [$-173$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$173$], [$[u p]$], [$519$], [$173$#super[d]], [$ZZ_(173)$], [$1$], [$1$],
  [$173$], [$[u p]$], [$519$], [$2$], [$(ZZ slash 2)^2$], [$2$#super[\*]], [$1$],
  [$179$], [$[1]$], [$1$], [$179$#super[d]], [$ZZ_(179)$], [$1$], [$1$],
  [$179$], [$[1]$], [$1$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$179$], [$[1]$], [$1$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$179$], [$[1]$], [$1$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$179$], [$[u]$], [$-19$], [$179$#super[d]], [$ZZ_(179)$], [$1$], [$1$],
  [$179$], [$[u]$], [$-19$], [$2$], [$C_(4)$], [$1$], [$1$],
  [$179$], [$[u]$], [$-19$], [$3$], [$C_(9)$], [$1$], [$1$],
  [$179$], [$[u]$], [$-19$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$179$], [$[p]$], [$179$], [$179$#super[d]], [$ZZ_(179)$], [$1$], [$1$],
  [$179$], [$[p]$], [$179$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$179$], [$[u p]$], [$-537$], [$179$#super[d]], [$ZZ_(179)$], [$1$], [$1$],
  [$179$], [$[u p]$], [$-537$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$181$], [$[1]$], [$1$], [$181$#super[d]], [$ZZ_(181)$], [$1$], [$1$],
  [$181$], [$[1]$], [$1$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$181$], [$[1]$], [$1$], [$5$], [$C_(5)$], [$1$], [$1$],
  [$181$], [$[1]$], [$1$], [$19$], [$C_(19)$], [$1$], [$1$],
  [$181$], [$[u]$], [$7$], [$181$#super[d]], [$ZZ_(181)$], [$1$], [$1$],
  [$181$], [$[u]$], [$7$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$181$], [$[u]$], [$7$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$181$], [$[u]$], [$7$], [$29$], [$C_(29)$], [$1$], [$1$],
  [$181$], [$[p]$], [$-181$], [$181$#super[d]], [$ZZ_(181)$], [$1$], [$1$],
  [$181$], [$[p]$], [$-181$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$181$], [$[u p]$], [$-1086$], [$181$#super[d]], [$ZZ_(181)$], [$1$], [$1$],
  [$181$], [$[u p]$], [$-1086$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$191$], [$[1]$], [$1$], [$191$#super[d]], [$ZZ_(191)$], [$1$], [$1$],
  [$191$], [$[1]$], [$1$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$191$], [$[1]$], [$1$], [$31$], [$C_(31)$], [$1$], [$1$],
  [$191$], [$[u]$], [$-1$], [$191$#super[d]], [$ZZ_(191)$], [$1$], [$1$],
  [$191$], [$[u]$], [$-1$], [$167$], [$C_(167)$], [$1$], [$1$],
  [$191$], [$[p]$], [$191$], [$191$#super[d]], [$ZZ_(191)$], [$1$], [$1$],
  [$191$], [$[u p]$], [$-191$], [$191$#super[d]], [$ZZ_(191)$], [$1$], [$1$],
  [$193$], [$[1]$], [$1$], [$193$#super[d]], [$ZZ_(193)$], [$1$], [$1$],
  [$193$], [$[1]$], [$1$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$193$], [$[1]$], [$1$], [$67$], [$C_(67)$], [$1$], [$1$],
  [$193$], [$[u]$], [$5$], [$193$#super[d]], [$ZZ_(193)$], [$1$], [$1$],
  [$193$], [$[u]$], [$5$], [$11$], [$C_(11)$], [$1$], [$1$],
  [$193$], [$[u]$], [$5$], [$17$], [$C_(17)$], [$1$], [$1$],
  [$193$], [$[p]$], [$-193$], [$193$#super[d]], [$ZZ_(193)$], [$1$], [$1$],
  [$193$], [$[u p]$], [$965$], [$193$#super[d]], [$ZZ_(193)$], [$1$], [$1$],
  [$197$], [$[1]$], [$-6$], [$197$#super[d]], [$ZZ_(197)$], [$1$], [$1$],
  [$197$], [$[1]$], [$-6$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$197$], [$[1]$], [$-6$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$197$], [$[1]$], [$-6$], [$37$], [$C_(37)$], [$1$], [$1$],
  [$197$], [$[u]$], [$3$], [$197$#super[d]], [$ZZ_(197)$], [$1$], [$1$],
  [$197$], [$[u]$], [$3$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$197$], [$[u]$], [$3$], [$3$], [$C_(3)$], [$1$], [$1$],
  [$197$], [$[u]$], [$3$], [$29$], [$C_(29)$], [$1$], [$1$],
  [$197$], [$[p]$], [$197$], [$197$#super[d]], [$ZZ_(197)$], [$1$], [$1$],
  [$197$], [$[p]$], [$197$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$197$], [$[u p]$], [$394$], [$197$#super[d]], [$ZZ_(197)$], [$1$], [$1$],
  [$197$], [$[u p]$], [$394$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$199$], [$[1]$], [$-6$], [$199$#super[d]], [$ZZ_(199)$], [$1$], [$1$],
  [$199$], [$[1]$], [$-6$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$199$], [$[1]$], [$-6$], [$109$], [$C_(109)$], [$1$], [$1$],
  [$199$], [$[u]$], [$3$], [$199$#super[d]], [$ZZ_(199)$], [$1$], [$1$],
  [$199$], [$[u]$], [$3$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$199$], [$[u]$], [$3$], [$7$], [$C_(7)$], [$1$], [$1$],
  [$199$], [$[u]$], [$3$], [$13$], [$C_(13)$], [$1$], [$1$],
  [$199$], [$[p]$], [$199$], [$199$#super[d]], [$ZZ_(199)$], [$1$], [$1$],
  [$199$], [$[p]$], [$199$], [$2$], [$C_(2)$], [$1$], [$1$],
  [$199$], [$[u p]$], [$-199$], [$199$#super[d]], [$ZZ_(199)$], [$1$], [$1$],
  [$199$], [$[u p]$], [$-199$], [$2$], [$C_(2)$], [$1$], [$1$],
)
#set text(size: 10.5pt)
