...
#block(fill: rgb("#fff4e6"), inset: 8pt, radius: 3pt, width: 100%)[
  *Status --- what is implemented is granularity $1$, and the rest is discarded.* Said plainly:
  `gran1` is an *admission test*, and a twist that fails it is skipped outright --- not stored at
  a coarser grade, not recorded for later, simply not entered. Granularity $1$ is exactly
  condition (ii) of @sec-local, tested by `gran1`/`hitsE1` in `ledger.gp`; every ledger run in
  this document admits on that test, so nothing below is a bare tally, and every run is also
  working with strictly less than it found.

  The discarding is *one-sided*, so soundness is not at issue: a refused twist can only cost
  coverage, never manufacture it. What it costs is measured in @sec-lprimary rather than guessed
  at --- of the 119 twists with points in the $d_0 = 1$ tuple, $17$ are refused, and *every one of
  them has finite granularity $2$*. None is inadmissible in the sense of @sec-ledger-graded; all
  17 would enter a level-2 ledger, where the termination theorem applies at $N = 2$. They are
  dropped for want of machinery, not on principle.

  The machinery they want is level 2, and that is where the flat representation dies: the arena
  is multiplied by $11 dot 13 dot 17 = 2431$, to $1.1 dot 10^7$ elements, and the triangular index
  computation costs $O(N)$ --- a direct attempt timed out. Hence the $ell$-primary rewrite, which
  @sec-lprimary now works out.
]
...
=== Designing the $ell$-primary rewrite <sec-lprimary>

@sec-layers says what the rewrite is for and leaves it there. This section works out what it
should actually be, and checks the central claim against the answer it has to reproduce
(`lprimary.gp`, output in `results/lprimary.txt`).

*The algebraic fact it rests on.* A subgroup of a finite abelian group is the direct sum of its
$ell$-primary components: $R = plus.big_ell R_ell$ with $R_ell = R inter cal(G)_ell$. So storing a
reach by its layer components loses *nothing* --- this is an exact change of representation, not
an approximation, and that is what distinguishes it from the level truncation, which is lossy and
needs the granularity datum to repair. Containment, intersection and the subgroup generated by a
set all decompose the same way, so every operation the star test performs is layerwise.

*What a reach should be.* Split the grid of @sec-layers into its two halves.

#table(
  columns: 3, align: (left, left, left), stroke: 0.4pt + luma(150),
  table.header([cell], [what it is], [how to store it]),
  [off-diagonal, $ell != p$],
    [$T_p [ell^infinity]$, *finite* and independent of the level --- nothing to refine],
    [a subgroup of $plus.big_p T_p [ell^infinity]$, by a generator matrix in Howell normal form
     over $ZZ slash ell^e$],
  [diagonal, $ell = p$],
    [$cal(G)_(p,p) tilde.equiv ZZ_p$ --- infinite, and the only thing that grows with $n$],
    [*one integer $k_p$*: the reach's $p$-layer is $p^(k_p) ZZ_p$],
)

The diagonal entry deserves the emphasis. $cal(G)_(p,p) tilde.equiv ZZ_p times T_p [p^infinity]$
in general, but @sec-cert-ppart establishes $T_p [p^infinity] = 0$ for every twist in play, so the
$p$-layer is a copy of $ZZ_p$ and a closed subgroup of it is $p^(k) ZZ_p$ --- a single integer,
*exact*, with no truncation anywhere. The level-$n$ truncation was never needed on the diagonal;
it was an artefact of representing everything by one flat bitmap.

*Granularity becomes a lookup.* With $v_p (M_p) = 0$, as holds for $S = {11,13,17}$, the arena's
$p$-layer is $E_1 (QQ_p)$ on the nose, so $R supset.eq E_n$ at $p$ reads $k_p <= n - 1$:
$ n_p = k_p + 1 . $
And $k_p$ is not a search. The $ZZ_p$-coordinate map is a homomorphism, so the closed span of the
generators' coordinates $alpha_i$ is $p^(min_i v_p (alpha_i)) ZZ_p$; with the depth formula of
@sec-cert-ppart, $v_p (alpha_i) = "depth"_i - 1$, whence
$ n_p = min_i "depth"_p (P_i) , $
the minimum of a column the certificates already print. `lprimary.gp` computes granularity both
ways --- this formula and `hitsE1`'s combination search --- on all 119 twists of the $d_0 = 1$
tuple: *no disagreements*.

*The star test, layer by layer.* Coverage does *not* decompose into independent per-layer
conditions, and it is worth being precise about why: a pair $(a,b)$ is covered when *one and the
same* entry contains it, so the choice must be constant across layers. Write, for each layer and
each subgroup $H subset.eq cal(G)_ell$ generated by at most two elements,
$ m_ell (H) := {R in cal(L) : H subset.eq R_ell} subset.eq cal(L) , $
a *mask*, i.e. a subset of the ledger. Then
$ cal(L) "covers every pair" quad <==> quad
  "for every choice of one mask per layer, " inter.big_ell m_ell != nothing . $
Only the *distinct* masks matter, and there are very few of them. This replaces an enumeration of
$|cal(G)(n)|^2$ ordered pairs by an enumeration of $product_ell |{m_ell}|$ mask tuples.

#block(fill: rgb("#eef4ff"), inset: 9pt, radius: 3pt, width: 100%)[
  *Run against the case that is already known.* For the seven index-2 reaches of
  @sec-tally-worked, $cal(G)(1)$ of order $4536 = 2^3 dot 3^4 dot 7$ splits as
  $cal(G)_2$ of order $8$, $cal(G)_3$ of order $81$, $cal(G)_7$ of order $7$.

  #table(columns: 4, align: (right, right, right, right), stroke: 0.4pt + luma(150),
    table.header([layer $ell$], [$|cal(G)_ell|$], [subgroups on $<= 2$ generators], [distinct masks]),
    [$2$], [$8$],  [$15$], [$15$],
    [$3$], [$81$], [$23$], [$1$],
    [$7$], [$7$],  [$2$],  [$1$],
  )

  Fifteen mask triples, none with empty intersection: *covered*, reproducing the bitmap verdict.
  And the shape of the answer is the point --- at $ell = 3$ and $ell = 7$ every $<= 2$-generated
  subgroup already lies in all seven reaches, so those layers contribute one mask each and cannot
  obstruct anything. The entire covering question is *15 subgroup tests inside a group of order
  $8$*, against the $4536^2 approx 2.1 dot 10^7$ ordered pairs the flat bitmap walks. That the
  binding layer is $ell = 2$ is exactly the $(ZZ slash 2)^3$ and its seven hyperplanes, now
  visible in the representation rather than inferred from a count.
]

*What level 2 then costs.* Here the rewrite pays for itself twice over.
$ker_1 slash ker_2 = product_p E_1 slash E_2$ has order $11 dot 13 dot 17 = 2431$, *coprime* to
$|cal(G)(1)| = 4536$, so the extension splits and
$ cal(G)(2) = cal(G)(1) ⊕ product_(p in S) ZZ slash p . $
The layers $ell = 2, 3, 7$ are *unchanged*. What level 2 adds is three *new* layers, each cyclic
of prime order $11$, $13$, $17$ --- and a subgroup of $C_p$ is $0$ or everything, so a reach's
datum in each is *one bit*. Level 2 costs three bits per entry, not a factor $2431$ on a bitmap.

That bit is not a new invention either: it is precisely "granularity $1$ at $p$". So in this
representation the admission test is not a bolted-on filter but the reach's own coordinate in the
$ell = p$ layer, and the entries `gran1` currently discards are the ones whose bit is $0$ --- data
the rewrite would simply *store* rather than refuse.

#block(fill: rgb("#fff4e6"), inset: 8pt, radius: 3pt, width: 100%)[
  *What remains to be done, and the one place to be careful.* The off-diagonal linear algebra over
  $ZZ slash ell^e$ is standard (Howell form), and the diagonal is an integer per place, so the
  data structures are undemanding. The work is in `reachmap`, which currently produces a bitmap
  and would have to produce layer components instead; the transport of a twist to the tuple's
  representative is unchanged.

  The care is needed at the *diagonal*, and specifically in the claim $k_p = min_i "depth"_i$.
  That argument used $T_p [p^infinity] = 0$ and $v_p (M_p) = 0$. The first holds throughout this
  document (@sec-cert-ppart) but *fails* for the CM curve of @sec-cm at $p = 61$, where the
  $p$-layer is $ZZ_61 times ZZ slash 61$ and a closed subgroup is no longer described by one
  integer. The second fails as soon as $p divides M_p$, which is every additive place --- and 56
  of the 64 tuples for this $S$ are ramified somewhere. A rewrite that means to sweep all 64 must
  therefore carry a genuine two-invariant description of the diagonal cell, not the single $k_p$
  that suffices for the tuple worked here.
]

=== Does the mod-$ell$ test survive a union? <sec-lprimary-union>

@sec-lprimary-why decides density at a single place by $R = cal(G)$ iff $R --> cal(G) slash ell$ is
onto for every $ell$, one $ell$ at a time. Here the object to be exhausted is not a reach but a
*union*, $U = union_d R(d) times R(d)$, and a union of subgroups is not a subgroup. Does the
reduction survive? *No* --- and for two independent reasons, which are worth separating because
only the second is the one @sec-lprimary already flagged. Both are exhibited in `coupling.gp` on
groups small enough to check by hand, since both are about the shape of the argument rather than
about size.

*First, what does transfer.* For a *single* twist the question "is $R(d)$ full?" is exactly the
question @sec-lprimary-why answers, and the Frattini reduction applies verbatim over $S$:
$R(d) = cal(G)$ iff $R(d) --> cal(G) slash ell cal(G)$ is onto for each $ell$, where
$ cal(G) slash ell cal(G) = plus.big_(p in S) E^(delta_p)(QQ_p) slash ell , quad
  dim_(FF_ell) cal(G) slash ell cal(G) = [ell in S] + sum_(p in S) dim E^(delta_p)(QQ_p)[ell] . $
That right-hand side is precisely the formula @sec-ledger gives for $g(arrow(delta))$. So the rank
dichotomy was the $ell$-primary statement all along, under another name: $g$ is the largest layer
of the Frattini quotient, and "$g <= 2$ forces a full twist" is the observation that a group with
all layers of dimension $<= 2$ is generated by any pair generating it modulo $Phi$.

*Second, what is still exactly valid.* A closed subgroup of a profinite abelian group is the
product of its $ell$-primary parts: $cal(G) = product_ell cal(G)_ell$ with $cal(G)_ell$ the pro-$ell$
Sylow, and a closed $R$ is itself profinite abelian, so $R = product_ell R_ell$ with
$R_ell = R inter cal(G)_ell$. Hence for a *fixed* $R$,
$ ⟨a, b⟩ subset.eq R quad <==> quad ⟨a_ell, b_ell⟩ subset.eq R_ell " for every " ell , $
exactly. Each individual containment does decompose. It is the two steps past this that fail.

#block(stroke: 0.6pt + black, inset: 9pt, radius: 3pt, width: 100%)[
  *Failure 1: membership is not a mod-$ell$ condition.* Frattini decides whether a subgroup is
  *everything*. Coverage asks whether particular *elements* lie in it, and modulo $ell$ one sees
  only $R + ell cal(G)$:
  $ overline(a) in overline(R) " in " cal(G) slash ell cal(G) quad <==> quad a in R + ell cal(G) , $
  in general strictly larger than $R$. Take $cal(G) = ZZ slash 4$, $R = 0$, $a = 2$: then
  $overline(a) = 0 = overline(R)$, so the mod-$ell$ test passes, while $a in.not R$. This already
  rules out transplanting @sec-lprimary-why verbatim --- there the mod-$ell$ quotient sufficed
  because the question was "is $R$ all of $cal(G)$", which is exactly what Frattini answers.
]

#block(stroke: 0.6pt + black, inset: 9pt, radius: 3pt, width: 100%)[
  *Failure 2: the existential does not commute with the layers.* Coverage reads
  $ forall (a,b) space exists R space forall ell : ⟨a_ell, b_ell⟩ subset.eq R_ell , $
  whereas checking layer by layer proves only
  $forall (a,b) space forall ell space exists R$, and the entry that works may *differ from layer
  to layer*. The smallest instance is $cal(G) = ZZ slash 6$ with the two entries $⟨3⟩$ and $⟨2⟩$:
  layer $2$ is covered, by $⟨3⟩$; layer $3$ is covered, by $⟨2⟩$; and $24$ of the $36$ ordered
  pairs --- $(0,1)$ among them --- lie in neither entry. Every layer covered, the group not.
]

*The repair is not to abandon the decomposition but to stop quantifying inside it.* For each layer
$ell$ and each $H subset.eq cal(G)_ell$ on at most two generators, keep the mask
$m_ell (H) = {R in cal(L) : H subset.eq R_ell}$. By the exactness above,
$ (a,b) " covered" quad <==> quad inter.big_ell m_ell (⟨a_ell, b_ell⟩) != nothing , $
so coverage holds iff no choice of one mask per layer has empty intersection. The layers still do
the work --- containment is tested in them --- but the existential is resolved *once*, at the end,
over the small finite set $cal(L)$. That is the star test of @sec-lprimary, and on the $ZZ slash 6$
example it returns the right answer: the masks are ${1}, {1,2}$ at $ell = 2$ and ${2}, {1,2}$ at
$ell = 3$, and the pair $({1}, {2})$ has empty intersection.

#block(fill: rgb("#fff4e6"), inset: 8pt, radius: 3pt, width: 100%)[
  *Why the coupling has never yet bitten --- which is data, not a theorem.* Every reach in every
  ledger in this document has index $1$ or $2$ in its arena: $66$ of index $2$, $14$ of index $1$,
  none other. An index-$2$ reach is full at every layer except $ell = 2$, so all layers $ell != 2$
  return the single full mask and cannot constrain the intersection --- the whole question
  collapses onto $ell = 2$, and the coupling is vacuous. That is why the prototype in
  @sec-lprimary reproduced the flat verdict exactly, and it should not be mistaken for the general
  case. The coupling would become active the moment one tuple carried an index-$2$ reach alongside
  an index-$3$ one, which is exactly the shape of Failure 2. Nothing forbids that; it has simply
  not occurred in the range searched.
]
...
witnesses with further columns, so that each line can be verified on its own:

+ the reduction data at $p$: the Kodaira type and the Tamagawa number $c_p$;
+ the torsion $E^d (QQ_p)_"tors" tilde.equiv C_a times C_b$. Only the torsion is tabulated: every
  line of the table has the same $ZZ_p$ in front of it, so printing it $180$ times says nothing,
  and the column heading carries the information instead;
...
The torsion column stands as printed. What changed is that it is now computed.
...
*How to read the base $g$ column.* It names the base of $T$ against which the last column's classes are discrete logarithms, and *two different kinds of entry share it*, exactly as
...
#table(columns: 2, align: (left, left), stroke: 0.4pt, inset: 6pt,
  ...
  [$u$; ord $2$ in $C_2$#super[\*]],
    [$T$ cyclic with no base to hand: the coordinate is given by its *order*, and the asterisk
     marks one whose order is $|T|$, i.e. a generator],
  [$u$; ---], [$T$ is trivial, so there is no second coordinate],
)]

#block(fill: rgb("#fff4e6"), inset: 8pt, radius: 3pt, width: 100%)[
  *Two different kinds of entry share this column, and the difference is easy to miss.* Where a
  base is printed, $t$ is an *element*, written as a discrete logarithm against that base: $(1)$
  is $1 dot g_1$, and $(6,1)$ is $6 g_1 + g_2$. Where no base is printed, $t$ is an *order* --- so
  "ord $2$ in $C_2$" means an element *of order $2$*, which in $C_2$ is the generator, not the
  element $2 = 0$. An earlier version of this table dropped the word "ord" and invited exactly
  that misreading.

  For the certificate the order is the relevant datum: a generator's image generates $T$ precisely
  when its order is $|T|$, which is what the asterisk marks. It also pins the element down
  completely whenever $|T| <= 2$, which covers all but three lines. On the remaining three ---
  $p = 31$ with $T tilde.equiv C_32$ and $C_30$, and $p = 31$, $d = -62$ with $T tilde.equiv C_4$
  --- an order determines the element only up to $"Aut"(C_n)$, which is all the certificate needs
  and all that is claimed. (Those three are lines of *multiplicative* or $I_1^*$ reduction, where
  $T$ is larger than the $c_p <= 4$ of the additive classes; $31$ is a bad prime of $E$ itself.)
]
...
#table(
  columns: 9,
  align: (right, center, right, center, center, left, center, left, left),
  stroke: 0.35pt + luma(175),
  inset: (x: 3.5pt, y: 2.5pt),
  table.header([$p$], [class], [$d$], [type], [$c_p$], [$E^d (QQ_p)_"tors"$], [base $g$],
               [$x(P)$], [image $(alpha; t)$]),
  [$3$], [$[1]$], [$7$], [$I_0$], [$1$], [$C_4$], [$(0,1)$], [$-3$], [$u$; $(1)$],
  [$3$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_4$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$3$], [$[p]$], [$3$], [$I_0^*$], [$2$], [$C_2$], [---], [$3$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$3$], [$[u p]$], [$6$], [$I_0^*$], [$2$], [$C_2$], [---], [$-3$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$5$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_9$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$5$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_3$], [$(3,1)$], [$3$], [$u$; $(2)$],
  [$5$], [$[p]$], [$5$], [$I_0^*$], [$1$], [---], [---], [$4$], [$u$; ---],
  [$5$], [$[u p]$], [$-35$], [$I_0^*$], [$1$], [---], [---], [$59004 slash 1369$], [$u$; ---],
  [$7$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_5$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$7$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_11$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$7$], [$[p]$], [$7$], [$I_0^*$], [$1$], [---], [---], [$-3$], [$u$; ---],
  [$7$], [$[u p]$], [$-7$], [$I_0^*$], [$1$], [---], [---], [$424 slash 25$], [$u$; ---],
  [$11$], [$[1]$], [$3$], [$I_0$], [$1$], [$C_14$], [$(1,2)$], [$3$], [$u$; $(3)$],
  [$11$], [$[u]$], [$6$], [$I_0$], [$1$], [$C_10$], [$(8,2)$], [$-3$], [$u$; $(9)$],
  [$11$], [$[p]$], [$11$], [$I_0^*$], [$2$], [$C_2$], [---], [$22$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$11$], [$[u p]$], [$-11$], [$I_0^*$], [$2$], [$C_2$], [---], [$33 slash 4$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$13$], [$[1]$], [$-1$], [$I_0$], [$1$], [$C_18$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$13$], [$[u]$], [$5$], [$I_0$], [$1$], [$C_10$], [$(2,1)$], [$4$], [$u$; $(3)$],
  [$13$], [$[p]$], [$-13$], [$I_0^*$], [$2$], [$C_2$], [---], [$247$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$13$], [$[u p]$], [$26$], [$I_0^*$], [$2$], [$C_2$], [---], [$13$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$17$], [$[1]$], [$-1$], [$I_0$], [$1$], [$C_18$], [$(0,4)$], [$1$], [$u$; $(13)$],
  [$17$], [$[u]$], [$7$], [$I_0$], [$1$], [$C_18$], [$(14,4)$], [$-3$], [$u$; $(17)$],
  [$17$], [$[p]$], [$34$], [$I_0^*$], [$2$], [$C_2$], [---], [$85$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$17$], [$[u p]$], [$51$], [$I_0^*$], [$2$], [$C_2$], [---], [$-17$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$19$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_21$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$19$], [$[u]$], [$-1$], [$I_0$], [$1$], [---], [---], [$1$], [$u$; ---],
  [$19$], [$[p]$], [$95$], [$I_0^*$], [$1$], [---], [---], [$59101 slash 121$], [$u$; ---],
  [$19$], [$[u p]$], [$-95$], [$I_0^*$], [$1$], [---], [---], [$3723268224 slash 6105841$], [$u$; ---],
  [$23$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_28$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$23$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_20$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$23$], [$[p]$], [$46$], [$I_0^*$], [$2$], [$C_2$], [---], [$3381 slash 625$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$23$], [$[u p]$], [$115$], [$I_0^*$], [$2$], [$C_2$], [---], [$-69$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$29$], [$[1]$], [$-1$], [$I_0$], [$1$], [$C_36$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$29$], [$[u]$], [$11$], [$I_0$], [$1$], [$C_24$], [$(4,9)$], [$22$], [$u$; $(5)$],
  [$29$], [$[p]$], [$-29$], [$I_0^*$], [$2$], [$C_2$], [---], [$87$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$29$], [$[u p]$], [$58$], [$I_0^*$], [$2$], [$C_2$], [---], [$-72682323 slash 1885129$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$31$], [$[1]$], [$1$], [$I_1$], [$1$], [$C_32$], [---], [$0$], [$u$; ord $32$ in $C_32$#super[\*]],
  [$31$], [$[u]$], [$-1$], [$I_1$], [$1$], [$C_30$], [---], [$1$], [$u$; ord $30$ in $C_30$#super[\*]],
  [$31$], [$[p]$], [$31$], [$I_1^*$], [$2$], [$C_2$], [---], [$93$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$31$], [$[u p]$], [$-62$], [$I_1^*$], [$4$], [$C_4$], [---], [$93$], [$u$; ord $4$ in $C_4$#super[\*]],
  [$37$], [$[1]$], [$-11$], [$I_0$], [$1$], [$C_48$], [$(1,7)$], [$220$], [$u$; $(19)$],
  [$37$], [$[u]$], [$6$], [$I_0$], [$1$], [$C_28$], [$(7,16)$], [$-3$], [$u$; $(5)$],
  [$37$], [$[p]$], [$-37$], [$I_0^*$], [$2$], [$C_2$], [---], [$8473 slash 16$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$37$], [$[u p]$], [$74$], [$I_0^*$], [$2$], [$C_2$], [---], [$91945 slash 1156$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$41$], [$[1]$], [$-1$], [$I_0$], [$1$], [$C_35$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$41$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_49$], [$(1,18)$], [$3$], [$u$; $(27)$],
  [$41$], [$[p]$], [$41$], [$I_0^*$], [$1$], [---], [---], [$-1328 slash 121$], [$u$; ---],
  [$41$], [$[u p]$], [$123$], [$I_0^*$], [$1$], [---], [---], [$9146571 slash 625$], [$u$; ---],
  [$43$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_34$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$43$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_54$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$43$], [$[p]$], [$-86$], [$I_0^*$], [$2$], [$C_2$], [---], [$26445 slash 169$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$43$], [$[u p]$], [$86$], [$I_0^*$], [$2$], [$C_2$], [---], [$-169979 slash 47089$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$47$], [$[1]$], [$-11$], [$I_0$], [$1$], [$C_30 times C_2$], [$(12,4),(2,0)$], [$33 slash 4$ #linebreak() $220$], [$u$; $(1, 1)$ #linebreak() $u$; $(28, 1)$],
  [$47$], [$[u]$], [$-149$], [$I_0$], [$1$], [$C_18 times C_2$], [$(3,6),(2,0)$], [$1639$ #linebreak() $8791 slash 25$], [$u$; $(12, 1)$ #linebreak() $u$; $(11, 1)$],
  [$47$], [$[p]$], [$94$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$3 p, 21 p$], [$141$ #linebreak() $1833 slash 16$], [$u$; $(1, 0)$ #linebreak() $u$; $(1, 1)$],
  [$47$], [$[u p]$], [$705$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$8 p, 40 p$], [$376$ #linebreak() $-2439159 slash 6400$], [$u$; $(1, 0)$ #linebreak() $u$; $(0, 1)$],
  [$53$], [$[1]$], [$11$], [$I_0$], [$1$], [$C_58$], [$(2,16)$], [$22$], [$u$; $(33)$],
  [$53$], [$[u]$], [$22$], [$I_0$], [$1$], [$C_50$], [$(2,21)$], [$2937 slash 196$], [$u$; $(3)$],
  [$53$], [$[p]$], [$53$], [$I_0^*$], [$2$], [$C_2$], [---], [$-10388 slash 289$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$53$], [$[u p]$], [$106$], [$I_0^*$], [$2$], [$C_2$], [---], [$25798969 slash 7396$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$59$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_63$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$59$], [$[u]$], [$6$], [$I_0$], [$1$], [$C_57$], [$(1,28)$], [$-3$], [$u$; $(46)$],
  [$59$], [$[p]$], [$295$], [$I_0^*$], [$1$], [---], [---], [$8251821 slash 49$], [$u$; ---],
  [$59$], [$[u p]$], [$-59$], [$I_0^*$], [$1$], [---], [---], [$217697199204 slash 5358093601$], [$u$; ---],
  [$61$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_50$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$61$], [$[u]$], [$7$], [$I_0$], [$1$], [$C_74$], [$(5,15)$], [$-3$], [$u$; $(59)$],
  [$61$], [$[p]$], [$-61$], [$I_0^*$], [$2$], [$C_2$], [---], [$10675 slash 81$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$61$], [$[u p]$], [$122$], [$I_0^*$], [$2$], [$C_2$], [---], [$2358325453 slash 110889$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$67$], [$[1]$], [$-221$], [$I_0$], [$1$], [$C_28 times C_2$], [$(4,30),(13,0)$], [$1326$ #linebreak() $1819 slash 9$], [$u$; $(20, 1)$ #linebreak() $u$; $(25, 0)$],
  [$67$], [$[u]$], [$51$], [$I_0$], [$1$], [$C_40 times C_2$], [$(1,28),(10,0)$], [$-17$ #linebreak() $-714 slash 25$], [$u$; $(9, 1)$ #linebreak() $u$; $(8, 1)$],
  [$67$], [$[p]$], [$2211$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$2 p, 27 p$], [$28743$ #linebreak() $4623$], [$u$; $(0, 1)$ #linebreak() $u$; $(1, 0)$],
  [$67$], [$[u p]$], [$134$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$26 p, 49 p$], [$-335 slash 4$ #linebreak() $94537 slash 196$], [$u$; $(0, 1)$ #linebreak() $u$; $(1, 0)$],
  [$71$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_59$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$71$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_85$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$71$], [$[p]$], [$71$], [$I_0^*$], [$1$], [---], [---], [$-47$], [$u$; ---],
  [$71$], [$[u p]$], [$-71$], [$I_0^*$], [$1$], [---], [---], [$334272 slash 6889$], [$u$; ---],
  [$73$], [$[1]$], [$3$], [$I_0$], [$1$], [$C_72$], [$(3,9)$], [$3$], [$u$; $(1)$],
  [$73$], [$[u]$], [$-21$], [$I_0$], [$1$], [$C_76$], [$(6,5)$], [$15$], [$u$; $(31)$],
  [$73$], [$[p]$], [$146$], [$I_0^*$], [$2$], [$C_2$], [---], [$7081 slash 64$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$73$], [$[u p]$], [$-365$], [$I_0^*$], [$2$], [$C_2$], [---], [$121326 slash 169$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$79$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_86$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$79$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_74$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$79$], [$[p]$], [$158$], [$I_0^*$], [$2$], [$C_2$], [---], [$5004161712613 slash 72071308521$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$79$], [$[u p]$], [$-158$], [$I_0^*$], [$2$], [$C_2$], [---], [$7760048840149 slash 57386598025$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$83$], [$[1]$], [$-22$], [$I_0$], [$1$], [$C_90$], [$(9,9)$], [$1177 slash 64$], [$u$; $(13)$],
  [$83$], [$[u]$], [$-11$], [$I_0$], [$1$], [$C_78$], [$(2,9)$], [$33 slash 4$ #linebreak() $220$], [$u$; $(51)$ #linebreak() $u$; $(13)$],
  [$83$], [$[p]$], [$83$], [$I_0^*$], [$2$], [$C_2$], [---], [$115702 slash 9$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$83$], [$[u p]$], [$166$], [$I_0^*$], [$2$], [$C_2$], [---], [$-7055 slash 64$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$89$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_100$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$89$], [$[u]$], [$-7$], [$I_0$], [$1$], [$C_80$], [$(3,3)$], [$424 slash 25$], [$u$; $(57)$],
  [$89$], [$[p]$], [$178$], [$I_0^*$], [$2$], [$C_2$], [---], [$50010613 slash 58081$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$89$], [$[u p]$], [$-267$], [$I_0^*$], [$2$], [$C_2$], [---], [$13439267253561 slash 39505537600$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$97$], [$[1]$], [$1$], [$I_0$], [$1$], [---], [---], [$0$], [$u$; ---],
  [$97$], [$[u]$], [$7$], [$I_0$], [$1$], [$C_99$], [$(2,35)$], [$-3$], [$u$; $(20)$],
  [$97$], [$[p]$], [$97$], [$I_0^*$], [$1$], [---], [---], [$-3020977264272 slash 60834742609$], [$u$; ---],
  [$97$], [$[u p]$], [$485$], [$I_0^*$], [$1$], [---], [---], [$607367181296288749204 slash 261353291940472081$], [$u$; ---],
  [$101$], [$[1]$], [$-1$], [$I_0$], [$1$], [$C_105$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$101$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_99$], [$(3,9)$], [$3$], [$u$; $(1)$],
  [$101$], [$[p]$], [$101$], [$I_0^*$], [$1$], [---], [---], [$-68$], [$u$; ---],
  [$101$], [$[u p]$], [$-2626$], [$I_0^*$], [$1$], [---], [---], [$80987149350133 slash 43587835729$], [$u$; ---],
  [$103$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_87$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$103$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_121$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$103$], [$[p]$], [$103$], [$I_0^*$], [$1$], [---], [---], [$1433661 slash 625$], [$u$; ---],
  [$103$], [$[u p]$], [$2266$], [$I_0^*$], [$1$], [---], [---], [$-811896233775 slash 535089424$], [$u$; ---],
  [$107$], [$[1]$], [$-7$], [$I_0$], [$1$], [$C_105$], [$(13,43)$], [$424 slash 25$], [$u$; $(46)$],
  [$107$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_111$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$107$], [$[p]$], [$-1605$], [$I_0^*$], [$1$], [---], [---], [$2571$], [$u$; ---],
  [$107$], [$[u p]$], [$-107$], [$I_0^*$], [$1$], [---], [---], [$348$], [$u$; ---],
  [$109$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_123$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$109$], [$[u]$], [$6$], [$I_0$], [$1$], [$C_97$], [$(1,12)$], [$-3$], [$u$; $(63)$],
  [$109$], [$[p]$], [$109$], [$I_0^*$], [$1$], [---], [---], [$33919060748052 slash 93717413689$], [$u$; ---],
  [$109$], [$[u p]$], [$654$], [$I_0^*$], [$1$], [---], [---], [$15870885 slash 6889$], [$u$; ---],
  [$113$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_125$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$113$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_103$], [$(2,36)$], [$3$], [$u$; $(13)$],
  [$113$], [$[p]$], [$113$], [$I_0^*$], [$1$], [---], [---], [$-160779984385773438752 slash 2170759046766859201$], [$u$; ---],
  [$113$], [$[u p]$], [$339$], [$I_0^*$], [$1$], [---], [---], [$-129$], [$u$; ---],
  [$127$], [$[1]$], [$-6$], [$I_0$], [$1$], [$C_126$], [$(15,4)$], [$21$], [$u$; $(121)$],
  [$127$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_130$], [$(3,9)$], [$3$], [$u$; $(1)$],
  [$127$], [$[p]$], [$254$], [$I_0^*$], [$2$], [$C_2$], [---], [$39963217 slash 20164$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$127$], [$[u p]$], [$-127$], [$I_0^*$], [$2$], [$C_2$], [---], [$3122041 slash 25600$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$131$], [$[1]$], [$53$], [$I_0$], [$1$], [$C_64 times C_2$], [$(0,42),(3,0)$], [$148$ #linebreak() $-10388 slash 289$], [$u$; $(3, 0)$ #linebreak() $u$; $(49, 1)$],
  [$131$], [$[u]$], [$-11$], [$I_0$], [$1$], [$C_68 times C_2$], [$(3,32),(76,0)$], [$33 slash 4$ #linebreak() $220$], [$u$; $(41, 1)$ #linebreak() $u$; $(25, 0)$],
  [$131$], [$[p]$], [$131$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$5 p, 51 p$], [$655$ #linebreak() $5371 slash 25$], [$u$; $(1, 0)$ #linebreak() $u$; $(1, 1)$],
  [$131$], [$[u p]$], [$8646$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$68 p, 91 p$], [$80565$ #linebreak() $-29168680211 slash 7425625$], [$u$; $(0, 1)$ #linebreak() $u$; $(1, 1)$],
  [$137$], [$[1]$], [$7$], [$I_0$], [$1$], [$C_126$], [$(5,24)$], [$-3$], [$u$; $(97)$],
  [$137$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_150$], [$(3,9)$], [$3$], [$u$; $(1)$],
  [$137$], [$[p]$], [$274$], [$I_0^*$], [$2$], [$C_2$], [---], [$331123731117 slash 5803544761$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$137$], [$[u p]$], [$-411$], [$I_0^*$], [$2$], [$C_2$], [---], [$1101728244 slash 3508129$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$139$], [$[1]$], [$51$], [$I_0$], [$1$], [$C_42 times C_3$], [$(2,23),(93,43)$], [$-17$ #linebreak() $-714 slash 25$], [$u$; $(34, 1)$ #linebreak() $u$; $(19, 2)$],
  [$139$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_154$], [$(1,68)$], [$3$], [$u$; $(71)$],
  [$139$], [$[p]$], [$139$], [$I_0^*$], [$2$], [$C_2$], [---], [$417 slash 4$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$139$], [$[u p]$], [$-139$], [$I_0^*$], [$2$], [$C_2$], [---], [$3892 slash 9$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$149$], [$[1]$], [$53$], [$I_0$], [$1$], [$C_68 times C_2$], [$(0,18),(13,0)$], [$148$ #linebreak() $-10388 slash 289$], [$u$; $(47, 0)$ #linebreak() $u$; $(38, 1)$],
  [$149$], [$[u]$], [$94$], [$I_0$], [$1$], [$C_82 times C_2$], [$(1,36),(40,0)$], [$141$ #linebreak() $1833 slash 16$], [$u$; $(50, 1)$ #linebreak() $u$; $(5, 0)$],
  [$149$], [$[p]$], [$-149$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$11 p, 56 p$], [$1639$ #linebreak() $8791 slash 25$], [$u$; $(1, 0)$ #linebreak() $u$; $(0, 1)$],
  [$149$], [$[u p]$], [$13559$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$42 p, 119 p$], [$20413$ #linebreak() $712634381069 slash 37982569$], [$u$; $(1, 1)$ #linebreak() $u$; $(0, 1)$],
  [$151$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_154$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$151$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_150$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$151$], [$[p]$], [$755$], [$I_0^*$], [$2$], [$C_2$], [---], [$151$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$151$], [$[u p]$], [$453$], [$I_0^*$], [$2$], [$C_2$], [---], [$3366104446853292 slash 32473665853489$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$157$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_171$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$157$], [$[u]$], [$5$], [$I_0$], [$1$], [$C_145$], [$(4,17)$], [$4$], [$u$; $(1)$],
  [$157$], [$[p]$], [$157$], [$I_0^*$], [$1$], [---], [---], [$4195788 slash 2809$], [$u$; ---],
  [$157$], [$[u p]$], [$2355$], [$I_0^*$], [$1$], [---], [---], [$22623639965396331 slash 4382453351761$], [$u$; ---],
  [$163$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_189$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$163$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_139$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$163$], [$[p]$], [$978$], [$I_0^*$], [$1$], [---], [---], [$933$], [$u$; ---],
  [$163$], [$[u p]$], [$815$], [$I_0^*$], [$1$], [---], [---], [$-2464911059 slash 18309841$], [$u$; ---],
  [$167$], [$[1]$], [$-13$], [$I_0$], [$1$], [$C_144$], [$(3,72)$], [$247$], [$u$; $(119)$],
  [$167$], [$[u]$], [$-6$], [$I_0$], [$1$], [$C_192$], [$(2,71)$], [$21$], [$u$; $(161)$],
  [$167$], [$[p]$], [$334$], [$I_0^*$], [$2$], [$C_2$], [---], [$58617 slash 4$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$167$], [$[u p]$], [$-334$], [$I_0^*$], [$2$], [$C_2$], [---], [$743828521 slash 1444804$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$173$], [$[1]$], [$51$], [$I_0$], [$1$], [$C_86 times C_2$], [$(0,47),(16,0)$], [$-17$ #linebreak() $-714 slash 25$], [$u$; $(41, 0)$ #linebreak() $u$; $(16, 1)$],
  [$173$], [$[u]$], [$53$], [$I_0$], [$1$], [$C_88 times C_2$], [$(2,35),(95,0)$], [$148$ #linebreak() $-10388 slash 289$], [$u$; $(39, 0)$ #linebreak() $u$; $(64, 1)$],
  [$173$], [$[p]$], [$-173$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$8 p, 37 p$], [$4671 slash 25$ #linebreak() $84078 slash 289$], [$u$; $(1, 0)$ #linebreak() $u$; $(0, 1)$],
  [$173$], [$[u p]$], [$519$], [$I_0^*$], [$4$], [$C_2 times C_2$], [$62 p, 135 p$], [$3633 slash 4$ #linebreak() $-1211 slash 25$], [$u$; $(0, 1)$ #linebreak() $u$; $(1, 0)$],
  [$179$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_180$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$179$], [$[u]$], [$-19$], [$I_0$], [$1$], [$C_180$], [$(6,78)$], [$9772 slash 361$], [$u$; $(161)$],
  [$179$], [$[p]$], [$179$], [$I_0^*$], [$2$], [$C_2$], [---], [$21659 slash 169$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$179$], [$[u p]$], [$-537$], [$I_0^*$], [$2$], [$C_2$], [---], [$1428490275232098 slash 2937758292121$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$181$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_190$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$181$], [$[u]$], [$7$], [$I_0$], [$1$], [$C_174$], [$(4,28)$], [$-3$], [$u$; $(73)$],
  [$181$], [$[p]$], [$-181$], [$I_0^*$], [$2$], [$C_2$], [---], [$1267 slash 9$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$181$], [$[u p]$], [$-1086$], [$I_0^*$], [$2$], [$C_2$], [---], [$113125 slash 121$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$191$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_217$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$191$], [$[u]$], [$-1$], [$I_0$], [$1$], [$C_167$], [$(1,1)$], [$1$], [$u$; $(1)$],
  [$191$], [$[p]$], [$191$], [$I_0^*$], [$1$], [---], [---], [$147421 slash 49$], [$u$; ---],
  [$191$], [$[u p]$], [$-191$], [$I_0^*$], [$1$], [---], [---], [$32463476400 slash 184334929$], [$u$; ---],
  [$193$], [$[1]$], [$1$], [$I_0$], [$1$], [$C_201$], [$(0,1)$], [$0$], [$u$; $(1)$],
  [$193$], [$[u]$], [$5$], [$I_0$], [$1$], [$C_187$], [$(4,17)$], [$4$], [$u$; $(1)$],
  [$193$], [$[p]$], [$-193$], [$I_0^*$], [$1$], [---], [---], [$9866360014417 slash 20332193281$], [$u$; ---],
  [$193$], [$[u p]$], [$965$], [$I_0^*$], [$1$], [---], [---], [$99428383084 slash 1100401$], [$u$; ---],
  [$197$], [$[1]$], [$-6$], [$I_0$], [$1$], [$C_222$], [$(2,88)$], [$21$], [$u$; $(85)$],
  [$197$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_174$], [$(2,29)$], [$3$], [$u$; $(91)$],
  [$197$], [$[p]$], [$197$], [$I_0^*$], [$2$], [$C_2$], [---], [$237188 slash 289$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$197$], [$[u p]$], [$394$], [$I_0^*$], [$2$], [$C_2$], [---], [$55718377785 slash 34175716$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$199$], [$[1]$], [$-6$], [$I_0$], [$1$], [$C_218$], [$(5,41)$], [$21$], [$u$; $(123)$],
  [$199$], [$[u]$], [$3$], [$I_0$], [$1$], [$C_182$], [$(3,9)$], [$3$], [$u$; $(1)$],
  [$199$], [$[p]$], [$199$], [$I_0^*$], [$2$], [$C_2$], [---], [$-995 slash 9$], [$u$; ord $2$ in $C_2$#super[\*]],
  [$199$], [$[u p]$], [$-199$], [$I_0^*$], [$2$], [$C_2$], [---], [$6169 slash 16$], [$u$; ord $2$ in $C_2$#super[\*]],
)
#set text(size: 10.5pt)

The $22$ lines carrying two generators are the ones where the single generator test failed. It is
tempting to add "because $E^d (QQ_p)$ is not procyclic there, so a rank-$1$ twist could never have
worked" --- an earlier version of this paragraph did --- but that is true of only $21$ of them.
@sec-lprimary-place separates the two statements and identifies the exception ($p = 83$, $d = -11$,
where the group *is* procyclic and a single point does generate; it is simply not either of the
two the search returned). The $34$ lines whose torsion column reads "---" are the additive
classes with $c_p = 1$, where the torsion vanishes entirely and $E^d (QQ_p) tilde.equiv ZZ_p$.
...
== The same sweep, $ell$-primary <sec-lprimary-place>

@sec-lprimary designs the $ell$-primary representation for the ledger, where it is needed because
the flat one dies at level 2. This section applies it to the sweep of @tab-primes and
@sec-cert-extended, where the flat one works perfectly well --- which is why it is the right place
to try the rewrite. `cert-extended.gp` is untouched; `cert-lprimary.gp` runs beside it and has to
reproduce its verdicts.

=== What the approach actually is <sec-lprimary-why>

#block(stroke: 0.6pt + black, inset: 9pt, radius: 3pt, width: 100%)[
  Let $G := E^d (QQ_p)$. It is profinite abelian and topologically finitely generated, so a
  *closed* subgroup $R$ equals $G$ as soon as $R dot Phi(G) = G$, where $Phi(G)$ is the Frattini
  subgroup --- the intersection of the maximal open subgroups. For abelian $G$ a maximal open
  subgroup has prime index, so $Phi(G)$ is the closure of the subgroup generated by all $ell$-th
  powers, and
  $ G slash Phi(G) = product_ell G slash ell G . $
  Only finitely many factors are nonzero: $G = ZZ_p times T$ gives
  $G slash ell G = ZZ_p slash ell times T slash ell T$, and $ell$ is invertible in $ZZ_p$ unless
  $ell = p$. So the relevant $ell$ are $p$ itself together with the primes dividing $\#T$. These
  factors have coprime orders, so surjecting onto the product is surjecting onto each. Hence
  $ E^d (QQ) " dense in " E^d (QQ_p) quad <==> quad
    E^d (QQ) --> E^d (QQ_p) slash ell " onto, for each such " ell , $
  and each $ell$ is checked *separately*.
]

That is the whole algorithm, and two remarks about it are worth making.

*It is Nakayama, in the form that applies here.* The pro-$p$ case is topological Nakayama proper;
what is used above is the Frattini argument, which is the same statement for a profinite abelian
group and needs no pro-$p$ hypothesis. The one point requiring care is that $R$ must be *closed* ---
which $R = overline(E^d (QQ))$ is by construction --- since the step "a proper closed subgroup lies
in a maximal open one" is what makes $Phi(G)$ do any work.

*"The primes dividing the order of $G$" has to be read as a supernatural number.* $G$ is infinite;
its order is $p^infinity dot \#T$. That is why $ell = p$ is always in play, even when $T$ is
trivial --- as it is on the two anomalous lines and the $34$ additive lines with $c_p = 1$, where
$ell = p$ is the *only* layer.

*Why the conditions are independent here, and are not for the ledger.* Coverage in @sec-ledger asks
that a *pair* lie in one and the same entry, so the choice of entry is coupled across layers and
the test becomes a mask intersection. Density of a single reach imposes no such coupling: the
conditions above really are independent, one per $ell$.

=== What the certificate must therefore show <sec-lprimary-cert>

Each $G slash ell G$ is an $FF_ell$-space of dimension at most $2$, so per triple $(p, d, ell)$ the
checkable content is exactly three things: a *basis* of $E^d (QQ_p) slash ell$; the *images* in that
basis of explicit generators of $E^d (QQ)$; and the *rank* of the resulting matrix over $FF_ell$,
the map being onto precisely when the rank is the dimension. Because the images are of the same
global points at every $ell$, those points are listed *once per pair $(p,d)$*, on a banner row, and
the $ell$-lines beneath refer back to them as $P_1, P_2$.

#block(fill: rgb("#fff4e6"), inset: 8pt, radius: 3pt, width: 100%)[
  The first version of this table reported only $|R_ell|$ against $|cal(G)_ell|$ --- the same
  verdict with the reasoning deleted. A reader could check that the two orders agreed only by
  redoing the computation; there was nothing on the page to verify against.
]

*The bases, by reduction type.* The form of a basis is the one place where the reduction type
still shows through.

#align(center)[
#table(columns: 2, align: (left, left), stroke: 0.4pt, inset: 6pt,
  [$ell = p$],
    [$T$ has no $p$-torsion (@sec-cert-ppart), so $T slash p T = 0$ and
     $G slash p G = ZZ_p slash p ZZ_p$ is *one-dimensional*. There is no canonical basis: the
     identification with $FF_p$ is only defined up to $FF_p^times$, since a topological generator
     of $ZZ_p$ must be chosen. So the only invariant content of an image is whether it *vanishes*,
     and it does not exactly when $v_p (alpha) = 0$, i.e. when $"depth"(M P) = 1$.],
  [$ell != p$, good],
    [$T = tilde(E)(FF_p) = C_(n_1) ⊕ C_(n_2)$, and with the canonical base $(g_1, g_2)$ of
     @sec-cert-conv the basis of $T slash ell T$ is the images of those $g_i$ with
     $ell divides n_i$. Coordinates here are discrete logarithms, so the base must be canonical or
     the column means nothing --- `ellgroup(E,p,1)` returns a *random* generator on each call. Its
     *structure* output is deterministic and is used freely.],
  [$ell != p$, additive],
    [$T = Phi(FF_p)$. When $Phi tilde.equiv (ZZ slash 2)^2$ the basis is the *$2$-torsion points*,
     which map isomorphically onto $Phi$; they are named by their $x$-coordinate, of valuation $1$,
     written $u p$ as in @tab-primes-extended.],
  [$ell != p$, multiplicative],
    [$T$ is cyclic and $T slash ell T$ is one-dimensional; the image of $P$ is its class reduced
     mod $ell$, nonzero exactly when that class is prime to $ell$.],
)]

#block(fill: rgb("#fff4e6"), inset: 8pt, radius: 3pt, width: 100%)[
  *A bug this classification fixed.* An earlier draft handled every non-good line as though
  $T = Phi(FF_p)$. That holds for *additive* reduction, where $E_0 slash E_1 tilde.equiv GG_a$ is a
  $p$-group and contributes nothing to $T$ --- but not for *multiplicative* reduction, where
  $E_0 slash E_1$ is the non-split torus, cyclic of order $p - a_p$ prime to $p$, and is the whole
  of $T$. The component group is then the wrong place to look, and both $p = 31$ lines with
  $31 divides.not d$ came back "not onto". $31$ is the bad prime of $E$ itself, so these are the
  only multiplicative lines in the table. The order of $overline(P)$ in $E slash E_1$ is the right
  invariant and needs no case analysis at all.
]

*It reproduces @tab-primes exactly*: all $180$ blocks are onto at every $ell$, no disagreements.

*And it sharpens the rank dichotomy into something visible.* $E^d (QQ_p)$ is procyclic exactly when
*every* $G slash ell G$ is one-dimensional --- a condition one now reads straight off the "dim"
column. @sec-cert-extended found $22$ lines on which no single generator is dense and filed them
all under "$E^d (QQ_p)$ is not procyclic". Only $21$ carry a two-dimensional layer (at
$p = 47, 67, 131, 139, 149, 173$, and always at $ell = 2$). The $22$nd is $p = 83$, class $[u]$,
$d = -11$, and its block reads

#align(center)[
#table(columns: 4, align: (right, center, left, left), stroke: 0.4pt, inset: 5pt,
  table.header([$ell$], [dim], [$P_1$], [$P_2$]),
  [$83$], [$1$], [$eq.not 0$], [$eq.not 0$],
  [$2$],  [$1$], [$1$], [$1$],
  [$3$],  [$1$], [*$0$*], [$1$],
  [$13$], [$1$], [$12$], [*$0$*],
)]

Every layer is one-dimensional, so the group *is* procyclic and some single point does generate;
the two the search returned merely *vanish at different $ell$* --- $P_1$ at $3$, $P_2$ at $13$.
Any combination avoiding both zeros works, for instance $-P_1 - 2 P_2$. In the flat presentation
this was a bare "index $3$" and "index $13$" with nothing to connect them; here the two zeros sit
in different rows of the same block and the remedy is obvious. "No generator *in our basis* is
dense" and "no point *of the group* is dense" are different statements, and @sec-cert-extended
conflated them.

=== The table <tab-lprimary>

A shaded banner row per pair $(p, d)$ gives the generators of $E^d (QQ)$ by their
$x$-coordinates; the lines below it are the triples $(p, d, ell)$ that use them. The layer marked
#super[d] is the *diagonal* one, $ell = p$. "onto" in the last column is
$E^d (QQ) --> E^d (QQ_p) slash ell$ surjective, i.e. rank $=$ dim; density at $p$ is the statement
that every line of a block says onto.

#set text(size: 7.2pt)
#table(
  columns: 6,
  align: (right, center, left, left, center, center),
  stroke: 0.35pt + luma(175),
  inset: (x: 4pt, y: 2.5pt),
    table.header([$ell$], [dim], [basis of $E^d (QQ_p) slash ell$], [images], [rank], [onto?]),
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 3$, class $[1]$, $d = 7$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$3$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 3$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$3$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 3$, class $[p]$, $d = 3$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 3$],
  [$3$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 3$, class $[u p]$, $d = 6$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = -3$],
  [$3$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 5$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$5$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 5$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$5$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(3,1)$], [$P_1 |-> (2)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 5$, class $[p]$, $d = 5$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 4$],
  [$5$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 5$, class $[u p]$, $d = -35$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 59004/1369$],
  [$5$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 7$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$7$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$5$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 7$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$7$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$11$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 7$, class $[p]$, $d = 7$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$7$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 7$, class $[u p]$, $d = -7$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 424/25$],
  [$7$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 11$, class $[1]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$11$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,2)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(1,2)$], [$P_1 |-> (3)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 11$, class $[u]$, $d = 6$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$11$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(8,2)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(8,2)$], [$P_1 |-> (4)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 11$, class $[p]$, $d = 11$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 22$],
  [$11$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 11$, class $[u p]$, $d = -11$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 33/4$, $x(P_2) = 220$],
  [$11$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 13$, class $[1]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$13$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 13$, class $[u]$, $d = 5$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 4$],
  [$13$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(2,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(2,1)$], [$P_1 |-> (3)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 13$, class $[p]$, $d = -13$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 247$],
  [$13$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 13$, class $[u p]$, $d = 26$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 13$],
  [$13$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 17$, class $[1]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$17$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,4)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(0,4)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 17$, class $[u]$, $d = 7$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$17$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(14,4)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(14,4)$], [$P_1 |-> (2)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 17$, class $[p]$, $d = 34$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 85$],
  [$17$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 17$, class $[u p]$, $d = 51$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = -17$, $x(P_2) = -714/25$],
  [$17$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 19$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$19$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 19$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$19$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 19$, class $[p]$, $d = 95$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 59101/121$],
  [$19$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 19$, class $[u p]$, $d = -95$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 3723268224/6105841$],
  [$19$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 23$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$23$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 23$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$23$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 23$, class $[p]$, $d = 46$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 3381/625$],
  [$23$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 23$, class $[u p]$, $d = 115$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = -69$],
  [$23$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 29$, class $[1]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$29$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 29$, class $[u]$, $d = 11$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 22$],
  [$29$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(4,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(4,9)$], [$P_1 |-> (2)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 29$, class $[p]$, $d = -29$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 87$],
  [$29$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 29$, class $[u p]$, $d = 58$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = -72682323/1885129$],
  [$29$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 31$, class $[1]$, $d = 1$ #h(8pt) $I_1$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$31$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(32)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 31$, class $[u]$, $d = -1$ #h(8pt) $I_1$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$31$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(30)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [gen. of $C_(30)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [gen. of $C_(30)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 31$, class $[p]$, $d = 31$ #h(8pt) $I_1^*$, $c_p = 2$ #h(8pt) $x(P_1) = 93$],
  [$31$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 31$, class $[u p]$, $d = -62$ #h(8pt) $I_1^*$, $c_p = 4$ #h(8pt) $x(P_1) = 93$],
  [$31$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(4)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 37$, class $[1]$, $d = -11$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 33/4$, $x(P_2) = 220$],
  [$37$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,7)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(1,7)$], [$P_1 |-> (0)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 37$, class $[u]$, $d = 6$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$37$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(7,16)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(7,16)$], [$P_1 |-> (5)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 37$, class $[p]$, $d = -37$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 8473/16$],
  [$37$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 37$, class $[u p]$, $d = 74$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 91945/1156$],
  [$37$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 41$, class $[1]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$41$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$5$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 41$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$41$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$7$], [$1$], [$(1,18)$], [$P_1 |-> (6)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 41$, class $[p]$, $d = 41$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -1328/121$],
  [$41$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 41$, class $[u p]$, $d = 123$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 9146571/625$],
  [$41$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 43$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$43$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$17$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 43$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$43$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 43$, class $[p]$, $d = -86$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 26445/169$],
  [$43$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 43$, class $[u p]$, $d = 86$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = -169979/47089$],
  [$43$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 47$, class $[1]$, $d = -11$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 33/4$, $x(P_2) = 220$],
  [$47$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(12,4)$, $(2,0)$], [$P_1 |-> (1, 1)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  [$3$], [$1$], [$(12,4)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(12,4)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (3)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 47$, class $[u]$, $d = -149$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1639$, $x(P_2) = 8791/25$],
  [$47$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(3,6)$, $(2,0)$], [$P_1 |-> (0, 1)$;#h(4pt) $P_2 |-> (1, 1)$], [$2$], [onto],
  [$3$], [$1$], [$(3,6)$], [$P_1 |-> (0)$;#h(4pt) $P_2 |-> (2)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 47$, class $[p]$, $d = 94$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 141$, $x(P_2) = 1833/16$],
  [$47$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(3 p, 0)$, $(21 p, 0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (1, 1)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 47$, class $[u p]$, $d = 705$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 376$, $x(P_2) = -2439159/6400$],
  [$47$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(8 p, 0)$, $(40 p, 0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 53$, class $[1]$, $d = 11$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 22$],
  [$53$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(2,16)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$29$], [$1$], [$(2,16)$], [$P_1 |-> (4)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 53$, class $[u]$, $d = 22$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 2937/196$],
  [$53$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(2,21)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(2,21)$], [$P_1 |-> (3)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 53$, class $[p]$, $d = 53$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 148$, $x(P_2) = -10388/289$],
  [$53$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (0)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 53$, class $[u p]$, $d = 106$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 25798969/7396$],
  [$53$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 59$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$59$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 59$, class $[u]$, $d = 6$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$59$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(1,28)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$19$], [$1$], [$(1,28)$], [$P_1 |-> (8)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 59$, class $[p]$, $d = 295$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 8251821/49$],
  [$59$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 59$, class $[u p]$, $d = -59$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 217697199204/5358093601$],
  [$59$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 61$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$61$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 61$, class $[u]$, $d = 7$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$61$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(5,15)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$37$], [$1$], [$(5,15)$], [$P_1 |-> (22)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 61$, class $[p]$, $d = -61$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 10675/81$],
  [$61$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 61$, class $[u p]$, $d = 122$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 2358325453/110889$],
  [$61$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 67$, class $[1]$, $d = -221$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1326$, $x(P_2) = 1819/9$],
  [$67$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(4,30)$, $(13,0)$], [$P_1 |-> (0, 1)$;#h(4pt) $P_2 |-> (1, 0)$], [$2$], [onto],
  [$7$], [$1$], [$(4,30)$], [$P_1 |-> (6)$;#h(4pt) $P_2 |-> (4)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 67$, class $[u]$, $d = 51$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -17$, $x(P_2) = -714/25$],
  [$67$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(1,28)$, $(10,0)$], [$P_1 |-> (1, 1)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  [$5$], [$1$], [$(1,28)$], [$P_1 |-> (4)$;#h(4pt) $P_2 |-> (3)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 67$, class $[p]$, $d = 2211$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 28743$, $x(P_2) = 4623$],
  [$67$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(2 p, 0)$, $(27 p, 0)$], [$P_1 |-> (0, 1)$;#h(4pt) $P_2 |-> (1, 0)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 67$, class $[u p]$, $d = 134$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = -335/4$, $x(P_2) = 94537/196$],
  [$67$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(26 p, 0)$, $(49 p, 0)$], [$P_1 |-> (0, 1)$;#h(4pt) $P_2 |-> (1, 0)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 71$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$71$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$59$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 71$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$71$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$5$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$17$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 71$, class $[p]$, $d = 71$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -47$],
  [$71$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 71$, class $[u p]$, $d = -71$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 334272/6889$],
  [$71$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 73$, class $[1]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$73$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 73$, class $[u]$, $d = -21$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 15$],
  [$73$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(6,5)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$19$], [$1$], [$(6,5)$], [$P_1 |-> (12)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 73$, class $[p]$, $d = 146$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 7081/64$],
  [$73$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 73$, class $[u p]$, $d = -365$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 121326/169$],
  [$73$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 79$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$79$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$43$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 79$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$79$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$37$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 79$, class $[p]$, $d = 158$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 5004161712613/72071308521$],
  [$79$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 79$, class $[u p]$, $d = -158$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 7760048840149/57386598025$],
  [$79$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 83$, class $[1]$, $d = -22$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1177/64$],
  [$83$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(9,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(9,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(9,9)$], [$P_1 |-> (3)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 83$, class $[u]$, $d = -11$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 33/4$, $x(P_2) = 220$],
  [$83$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(2,9)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(2,9)$], [$P_1 |-> (0)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  [$13$], [$1$], [$(2,9)$], [$P_1 |-> (12)$;#h(4pt) $P_2 |-> (0)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 83$, class $[p]$, $d = 83$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 115702/9$],
  [$83$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 83$, class $[u p]$, $d = 166$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = -7055/64$],
  [$83$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 89$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$89$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 89$, class $[u]$, $d = -7$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 424/25$],
  [$89$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(3,3)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(3,3)$], [$P_1 |-> (2)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 89$, class $[p]$, $d = 178$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 50010613/58081$],
  [$89$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 89$, class $[u p]$, $d = -267$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 13439267253561/39505537600$],
  [$89$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 97$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$97$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 97$, class $[u]$, $d = 7$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$97$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(2,35)$], [$P_1 |-> (2)$], [$1$], [onto],
  [$11$], [$1$], [$(2,35)$], [$P_1 |-> (9)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 97$, class $[p]$, $d = 97$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -3020977264272/60834742609$],
  [$97$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 97$, class $[u p]$, $d = 485$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 607367181296288749204/261353291940472081$],
  [$97$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 101$, class $[1]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$101$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 101$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$101$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$11$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 101$, class $[p]$, $d = 101$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -68$],
  [$101$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 101$, class $[u p]$, $d = -2626$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 80987149350133/43587835729$],
  [$101$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 103$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$103$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$29$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 103$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$103$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$11$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 103$, class $[p]$, $d = 103$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 1433661/625$],
  [$103$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 103$, class $[u p]$, $d = 2266$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -811896233775/535089424$],
  [$103$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 107$, class $[1]$, $d = -7$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 424/25$],
  [$107$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(13,43)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(13,43)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(13,43)$], [$P_1 |-> (4)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 107$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$107$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$37$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 107$, class $[p]$, $d = -1605$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 2571$],
  [$107$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 107$, class $[u p]$, $d = -107$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 348$],
  [$107$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 109$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$109$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$41$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 109$, class $[u]$, $d = 6$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$109$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$97$], [$1$], [$(1,12)$], [$P_1 |-> (63)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 109$, class $[p]$, $d = 109$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 33919060748052/93717413689$],
  [$109$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 109$, class $[u p]$, $d = 654$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 15870885/6889$],
  [$109$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 113$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$113$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$5$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 113$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$113$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$103$], [$1$], [$(2,36)$], [$P_1 |-> (13)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 113$, class $[p]$, $d = 113$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -160779984385773438752/2170759046766859201$],
  [$113$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 113$, class $[u p]$, $d = 339$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -129$],
  [$113$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 127$, class $[1]$, $d = -6$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 21$],
  [$127$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(15,4)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(15,4)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(15,4)$], [$P_1 |-> (2)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 127$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$127$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$13$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 127$, class $[p]$, $d = 254$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 39963217/20164$],
  [$127$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 127$, class $[u p]$, $d = -127$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 88$, $x(P_2) = 3122041/25600$],
  [$127$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (0)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 131$, class $[1]$, $d = 53$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 148$, $x(P_2) = -10388/289$],
  [$131$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(0,42)$, $(3,0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (1, 1)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 131$, class $[u]$, $d = -11$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 33/4$, $x(P_2) = 220$],
  [$131$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(3,32)$, $(76,0)$], [$P_1 |-> (1, 1)$;#h(4pt) $P_2 |-> (1, 0)$], [$2$], [onto],
  [$17$], [$1$], [$(3,32)$], [$P_1 |-> (7)$;#h(4pt) $P_2 |-> (8)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 131$, class $[p]$, $d = 131$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 655$, $x(P_2) = 5371/25$],
  [$131$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(5 p, 0)$, $(51 p, 0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (1, 1)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 131$, class $[u p]$, $d = 8646$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 80565$, $x(P_2) = -29168680211/7425625$],
  [$131$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(68 p, 0)$, $(91 p, 0)$], [$P_1 |-> (0, 1)$;#h(4pt) $P_2 |-> (1, 1)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 137$, class $[1]$, $d = 7$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$137$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(5,24)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(5,24)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(5,24)$], [$P_1 |-> (6)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 137$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$137$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 137$, class $[p]$, $d = 274$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 331123731117/5803544761$],
  [$137$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 137$, class $[u p]$, $d = -411$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 1101728244/3508129$],
  [$137$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 139$, class $[1]$, $d = 51$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -17$, $x(P_2) = -714/25$],
  [$139$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(2,23)$], [$P_1 |-> (0)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  [$3$], [$2$], [$(2,23)$, $(93,43)$], [$P_1 |-> (1, 1)$;#h(4pt) $P_2 |-> (1, 2)$], [$2$], [onto],
  [$7$], [$1$], [$(2,23)$], [$P_1 |-> (6)$;#h(4pt) $P_2 |-> (5)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 139$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$139$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,68)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(1,68)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$11$], [$1$], [$(1,68)$], [$P_1 |-> (5)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 139$, class $[p]$, $d = 139$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 417/4$],
  [$139$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 139$, class $[u p]$, $d = -139$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 3892/9$, $x(P_2) = 148204$],
  [$139$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (0)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 149$, class $[1]$, $d = 53$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 148$, $x(P_2) = -10388/289$],
  [$149$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(0,18)$, $(13,0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  [$17$], [$1$], [$(0,18)$], [$P_1 |-> (13)$;#h(4pt) $P_2 |-> (4)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 149$, class $[u]$, $d = 94$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 141$, $x(P_2) = 1833/16$],
  [$149$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(1,36)$, $(40,0)$], [$P_1 |-> (0, 1)$;#h(4pt) $P_2 |-> (1, 0)$], [$2$], [onto],
  [$41$], [$1$], [$(1,36)$], [$P_1 |-> (9)$;#h(4pt) $P_2 |-> (5)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 149$, class $[p]$, $d = -149$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 1639$, $x(P_2) = 8791/25$],
  [$149$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(11 p, 0)$, $(56 p, 0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 149$, class $[u p]$, $d = 13559$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 20413$, $x(P_2) = 712634381069/37982569$],
  [$149$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(42 p, 0)$, $(119 p, 0)$], [$P_1 |-> (1, 1)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 151$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$151$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$11$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 151$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$151$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 151$, class $[p]$, $d = 755$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 151$],
  [$151$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 151$, class $[u p]$, $d = 453$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 3366104446853292/32473665853489$],
  [$151$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 157$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$157$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$19$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 157$, class $[u]$, $d = 5$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 4$],
  [$157$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$5$], [$1$], [$(4,17)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$29$], [$1$], [$(4,17)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 157$, class $[p]$, $d = 157$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 4195788/2809$],
  [$157$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 157$, class $[u p]$, $d = 2355$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 22623639965396331/4382453351761$],
  [$157$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 163$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$163$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 163$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$163$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$139$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 163$, class $[p]$, $d = 978$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 933$],
  [$163$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 163$, class $[u p]$, $d = 815$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = -2464911059/18309841$],
  [$163$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 167$, class $[1]$, $d = -13$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 247$],
  [$167$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(3,72)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(3,72)$], [$P_1 |-> (2)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 167$, class $[u]$, $d = -6$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 21$],
  [$167$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(2,71)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(2,71)$], [$P_1 |-> (2)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 167$, class $[p]$, $d = 334$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 58617/4$],
  [$167$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 167$, class $[u p]$, $d = -334$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 743828521/1444804$],
  [$167$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 173$, class $[1]$, $d = 51$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -17$, $x(P_2) = -714/25$],
  [$173$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(0,47)$, $(16,0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  [$43$], [$1$], [$(0,47)$], [$P_1 |-> (41)$;#h(4pt) $P_2 |-> (16)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 173$, class $[u]$, $d = 53$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 148$, $x(P_2) = -10388/289$],
  [$173$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(2,35)$, $(95,0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  [$11$], [$1$], [$(2,35)$], [$P_1 |-> (6)$;#h(4pt) $P_2 |-> (9)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 173$, class $[p]$, $d = -173$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 4671/25$, $x(P_2) = 84078/289$],
  [$173$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(8 p, 0)$, $(37 p, 0)$], [$P_1 |-> (1, 0)$;#h(4pt) $P_2 |-> (0, 1)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 173$, class $[u p]$, $d = 519$ #h(8pt) $I_0^*$, $c_p = 4$ #h(8pt) $x(P_1) = 3633/4$, $x(P_2) = -1211/25$],
  [$173$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$2$], [$(62 p, 0)$, $(135 p, 0)$], [$P_1 |-> (0, 1)$;#h(4pt) $P_2 |-> (1, 0)$], [$2$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 179$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$179$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 179$, class $[u]$, $d = -19$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 9772/361$],
  [$179$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(6,78)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(6,78)$], [$P_1 |-> (2)$], [$1$], [onto],
  [$5$], [$1$], [$(6,78)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 179$, class $[p]$, $d = 179$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 21659/169$],
  [$179$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 179$, class $[u p]$, $d = -537$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 1428490275232098/2937758292121$],
  [$179$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 181$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$181$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$5$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$19$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 181$, class $[u]$, $d = 7$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = -3$],
  [$181$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(4,28)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(4,28)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$29$], [$1$], [$(4,28)$], [$P_1 |-> (15)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 181$, class $[p]$, $d = -181$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 1267/9$],
  [$181$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 181$, class $[u p]$, $d = -1086$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 113125/121$, $x(P_2) = 1072425/1024$],
  [$181$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 191$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$191$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$7$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$31$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 191$, class $[u]$, $d = -1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 1$],
  [$191$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$167$], [$1$], [$(1,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 191$, class $[p]$, $d = 191$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 147421/49$],
  [$191$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 191$, class $[u p]$, $d = -191$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 32463476400/184334929$],
  [$191$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 193$, class $[1]$, $d = 1$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 0$],
  [$193$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$3$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$67$], [$1$], [$(0,1)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 193$, class $[u]$, $d = 5$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 4$],
  [$193$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$11$], [$1$], [$(4,17)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$17$], [$1$], [$(4,17)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 193$, class $[p]$, $d = -193$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 9866360014417/20332193281$],
  [$193$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 193$, class $[u p]$, $d = 965$ #h(8pt) $I_0^*$, $c_p = 1$ #h(8pt) $x(P_1) = 99428383084/1100401$],
  [$193$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 197$, class $[1]$, $d = -6$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 21$],
  [$197$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(2,88)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(2,88)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$37$], [$1$], [$(2,88)$], [$P_1 |-> (11)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 197$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$197$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(2,29)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$3$], [$1$], [$(2,29)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$29$], [$1$], [$(2,29)$], [$P_1 |-> (4)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 197$, class $[p]$, $d = 197$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 237188/289$, $x(P_2) = -398636/3025$],
  [$197$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (0)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 197$, class $[u p]$, $d = 394$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 55718377785/34175716$],
  [$197$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 199$, class $[1]$, $d = -6$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 21$],
  [$199$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(5,41)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$109$], [$1$], [$(5,41)$], [$P_1 |-> (14)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 199$, class $[u]$, $d = 3$ #h(8pt) $I_0$, $c_p = 1$ #h(8pt) $x(P_1) = 3$],
  [$199$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$7$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  [$13$], [$1$], [$(3,9)$], [$P_1 |-> (1)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 199$, class $[p]$, $d = 199$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = -995/9$, $x(P_2) = 33$],
  [$199$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (0)$], [$1$], [onto],
  table.cell(colspan: 6, fill: luma(242))[#h(1pt) $p = 199$, class $[u p]$, $d = -199$ #h(8pt) $I_0^*$, $c_p = 2$ #h(8pt) $x(P_1) = 6169/16$, $x(P_2) = 18144424/109561$],
  [$199$#super[d]], [$1$], [--- (dim $1$)], [$P_1 |-> eq.not 0$;#h(4pt) $P_2 |-> eq.not 0$], [$1$], [onto],
  [$2$], [$1$], [gen. of $C_(2)$], [$P_1 |-> (1)$;#h(4pt) $P_2 |-> (0)$], [$1$], [onto],
)
#set text(size: 10.5pt)
