...
| `selmer-involution.typ` | **why the 2-Selmer ranks of `y^2=x^3+a` and `y^2=x^3-1/a` agree** (MathOverflow 227987, 2015). The curves are non-isogenous but `theta -> -1/theta` is an isomorphism of the étale algebras, so they share their mod-2 Galois module and both Selmer groups live in the same `H^1(Q,M)` -- which is why the ranks are correlated. Reduces the conjecture to equality of the local conditions, automatic at good places and at infinity, and localises the remaining content at `v = 2` (where the even-`a` counterexamples all differ by exactly 1). |
