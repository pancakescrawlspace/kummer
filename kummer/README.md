...
| `localimg.gp` | **the local statements of §6, made exhaustive.** Each is about `E_d` over one `Q_v`, and `E_d/Q_v` depends on `d` only through its class in `Q_v^x` mod squares (4 classes for odd `v`, 8 for `v=2`), so a bounded number of local runs settles every squarefree `d`. Each run is a proof, not a sample: the sampled points are shown to *generate* the relevant quotient, and value classes are read off from the valuation and finitely many digits. Currently: 14a1 at `v=7`, where the image of `(c_1,c_2)` on `W_7` has 9 elements and is the antidiagonal. `cubcls`, `gen1`, `run7`. Output: `results/survey-localimg.txt`. |
