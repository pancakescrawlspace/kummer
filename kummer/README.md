...
| `localimg.gp` | **the local statements of §6, made exhaustive.** Each is about `E_d` over one `Q_v`, and `E_d/Q_v` depends on `d` only through its class in `Q_v^x` mod squares (4 classes for odd `v`, 8 for `v=2`), so a bounded number of local runs settles every squarefree `d`. Each run is a proof, not a sample: the sampled points are shown to *generate* the relevant quotient, and value classes are read off from the valuation and finitely many digits. Covers 14a1 at `v=7` (image of `(c_1,c_2)` on `W_7` has 9 elements, antidiagonal) and 15a4 at `v=5` (image of `c` is all four classes) and `v=2` (order at most 2, all eight classes of `d`). `cubcls`, `sqcls`, `gen1`, `genq`, `run7`, `runc`. Output: `results/survey-localimg.txt`. |
