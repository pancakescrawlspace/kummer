...
Not proved: sufficiency (verified in nine cases, not proved), the Tate step 3
and the `C_can` characterisation (standard, but not written out). Out of scope:
the wild place `v = ell`, where `x^3-2`'s obstruction lives, and at `ell = 2`
the places `infinity` and `q | d`.
