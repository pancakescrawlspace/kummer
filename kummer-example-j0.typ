...
#block(fill: luma(240), inset: 8pt, radius: 3pt, width: 100%)[
  *Summary.* At $p = 3, 5, 7$ a single twist covers every square class, so $X(QQ)$ is dense in
  ...
  eight are settled (@sec-obs-nosample). Hence $X(QQ)$ is not dense in $X(QQ_2)$. @sec-brauer packages all of this as a single Azumaya
  algebra $cal(A) = "cor"_(K(X) slash QQ(X))(x^2 - u^2 x + 3u, thin t^2 + 3u t + 9u^2)$ on $X$,
  $K = QQ(u)$ with $u^3 = 3$, whose local invariants are the $beta_q$ and in which the twist does
  not appear. $E_d (QQ_2)$ always has a $2$-torsion point while $E_d (QQ)$ never
...
$beta$ is a globally defined pairing, it obeys a reciprocity law (@sec-obs-formula), and it
vanishes at every place but one (@sec-obs-status). That is the signature of an Azumaya algebra on
$X$, and this section writes it down. Nothing here is a new theorem about the example: it is the
same content, repackaged so that the twist disappears and the eight square classes of
@sec-obs-nosample become the eight values of *one function on $X(QQ_2)$*.

#block(fill: luma(240), inset: 8pt, radius: 3pt, width: 100%)[
  *Two things to say first, because they are what makes this case different from the
  $E times E$ ones of `kummer-survey.typ` §7.*

  #v(2mm)
  *(i) $beta$ is not a twisted pairing.* It is the *ordinary* local Tate pairing on
  $H^1(QQ_v, E[2])$, evaluated on one class from $E_d$ and one transported from $E'_d$ along
  $psi$. In the $E times E$ setting a *non-scalar* $phi in "End"_G (E[2])$ is essential: both
  arguments come from the same curve, $W_v$ is Lagrangian, and a scalar $phi$ gives zero
  identically. Here that mechanism is not available and not needed --- $psi$ is the unique non-zero
  element of a one-dimensional $"Hom"$ space, "scalar" in the only sense available, and $beta$ is
  still non-zero, because $L_2 (E_d)$ and $psi_* L'_2 (E'_d)$ are *two different Lagrangians*.
  The second curve does the job the twisting endomorphism did there.

  #v(2mm)
  *(ii) Non-isogeny costs nothing.* The construction $cal(A)_phi = e_(n*)(p_1^* tau union phi_*
  p_2^* tau)$ of `kummer-survey.typ` §7.2 never uses $E = E'$: replace $E times E$ by
  $E times E'$, $tau$ by $tau_E$ and $tau_(E')$, and $phi in "End"_G (E[n])$ by
  $psi in "Hom"_G (E'[n], E[n])$, and every step goes through verbatim --- it is functoriality
  throughout. Non-isogeny enters only as $"Hom"(E', E) = 0$, which is what makes *every* non-zero
  $psi$ admissible (@sec-obs-A). What genuinely differs is @sec-brauer-field.
]

== One cubic field, for every twist <sec-brauer-field>

$U^3 + 9 d^3$ has no rational root (@sec-obs-three, or simply that $9d^3$ is never minus a cube),
so $E_d [2]$ is the sum-zero submodule of the permutation module on the three roots, and the
$2$-descent étale algebra is
$ A_d = QQ[U] slash (U^3 + 9 d^3) tilde.equiv QQ(u), quad u^3 = 3 , $
*independent of $d$*, because $d^3$ is a cube and $-9 = -u^6$. The same field serves $E'_d$. And
by the Theorem of `corestriction.typ` §5, the local Tate pairing on such a descent module *is* a
sum of Hilbert symbols over the places of the étale algebra. So the formula of @sec-obs-formula,
$ beta_q (P,P') = sum_(w divides q) ( x(P) + d u^2, thin x(P') - 3 d u )_(K_w) ,
  quad K = QQ(u) , $
was already a corestriction from $K$; it was just not called one.

#block(stroke: 0.6pt + black, inset: 9pt, radius: 3pt, width: 100%)[
  *Why the corestriction is forced here.* $"Gal"(QQ(root(3,3), zeta_3) slash QQ) = S_3$ acts on
  $E[2]$ as the full $"GL"_2 (bb(F)_2)$, so $E[2]$ is *irreducible* over $QQ$: there is no
  Galois-stable line, hence no descent function over $QQ$, hence no plain quaternion symbol over
  $QQ(X)$. Over the cubic $K$ the residual group is $ZZ slash 2$ and there is exactly one stable
  line --- generated by the point with $x$-coordinate $-d u^2$. Degree $3$ is coprime to $2$, so
  the class descends.

  #v(2mm)
  Two consequences. $"End"_G (E[2]) = bb(F)_2$, so $"Hom"_G (E'[2], E[2])$ is *one-dimensional*:
  unlike `15a1`, where nine rank-one $phi$ had to be sifted, there is exactly one candidate class
  and no search. And the corestriction, invisible at $ell = 2$ in `kummer-survey.typ` §7.7 and
  appearing only at $ell = 3$ there, is unavoidable at $ell = 2$ here.
]

== The algebra <sec-brauer-alg>

Over $K$ both cubics factor, one linear factor and one quadratic:
$ x^3 + 9 = (x + u^2) space g(x), quad & g(x) = x^2 - u^2 x + 3 u , \
  t^3 - 81 = (t - 3u) space h(t), quad & h(t) = t^2 + 3 u t + 9 u^2 , $
using $u^4 = 3u$ and $u^6 = 9$.

#block(fill: luma(240), inset: 8pt, radius: 3pt, width: 100%)[
  $ cal(A) := "cor"_(K(X) slash QQ(X)) space ( space x^2 - u^2 x + 3u, quad
    t^2 + 3 u t + 9 u^2 space ) space in space "Br"(X) , $
  on $X : y^2 = (x^3+9)(t^3-81)$, with $K = QQ(u)$, $u^3 = 3$.
]

The entries are the *complementary* factors, not the linear ones, and that is the whole trick ---
it is what removes the twist. The linear factor is the descent class: on $E_d$ the function
$U + d u^2$ has divisor $2(T_0) - 2(O)$, and it is $x(P) + d u^2$ of @sec-obs-formula. But it
mentions $d$. Now on $E_d$,
$ V^2 = U^3 + 9d^3 = (U + d u^2) space Q(U), quad Q(U) = U^2 - d u^2 U + d^2 u^4 , $
so $U + d u^2 equiv Q(U)$ modulo squares; and under the map $E_d --> X$, which is $U = d x$,
$ Q(d x) = d^2 x^2 - d^2 u^2 x + d^2 u^4 = d^2 space g(x) . $
The $d^2$ is a square and disappears:
$ x(P) + d u^2 space equiv space g(x) quad (mod " squares in " K^times) . $
Identically the same on the other side, giving $x(P') - 3 d u equiv h(t)$. This is the exact
analogue of `kummer-survey.typ` §7.8's "each factor is congruent to the product of the other two",
with one difference: there the complementary factor was a product of two *linear* forms over $QQ$,
because $E[2]$ was decomposable; here only one root is rational over $K$, so the complementary
factor is a single *quadratic*.

Checked as an exact identity, not modulo squares: $(U + d u^2) space g(U slash d) = (V slash d)^2$
on every one of $71$ integral points found on the twists.

== It evaluates to $beta$ <sec-brauer-eval>

Corestriction preserves local invariants placewise --- $"inv"_v compose "cor" = sum_(w divides v)
"inv"_w$, property (P4) of `corestriction.typ` --- so for $T in X(QQ_q)$ the image of $(P, P')$,
$ "inv"_q cal(A)(T) = sum_(w divides q) ( g(x), h(t) )_(K_w)
  = sum_(w divides q) ( x(P) + d u^2, thin x(P') - 3 d u )_(K_w) = beta_q (P, P') . $

#block(stroke: 0.6pt + black, inset: 9pt, radius: 3pt, width: 100%)[
  *The corestriction does the matching of embeddings for free.* @sec-obs-formula had to check by
  hand that $iota_k (-d u^2) = r_(2k)$ and $iota_k (3 d u) = s_k$, so that pairing the two entries
  *inside the same copy of $K$* realises $psi(s_j) = r_(2j)$. That bookkeeping is exactly what a
  corestriction is: it sums over the places of one $K$ with both entries in that same $K$. The
  identification $psi$ is not an extra input to $cal(A)$; it is the choice of which root of
  $t^3 - 81$ to call linear, i.e. the choice of $h$.
]

Verified against the twisted formula of @sec-obs-formula on $43,670$ instances --- twelve twists,
four primes, sampled local points on both curves --- with no disagreement.
...
A point $T = (x,t,y) in X(QQ_v)$ lifts to $E_d times E'_d$ exactly for
$ d equiv x^3 + 9 equiv t^3 - 81 quad (mod " squares") , $
the two agreeing because their product is $y^2$. So *the square class of the twist is a function of
the point*, and the eight classes of @sec-obs-nosample are eight open subsets of $X(QQ_2)$.

#block(stroke: 0.6pt + black, inset: 9pt, radius: 3pt, width: 100%)[
  *The whole of @sec-obstruction, in three lines about $cal(A)$.*
  #v(1mm)
  + $"inv"_q cal(A) equiv 0$ on $X(QQ_q)$ for every $q != 2$, including $q = infinity$.
  + $"inv"_2 cal(A) : X(QQ_2) --> 1/2 ZZ slash ZZ$ is locally constant and *not constant*: it
    vanishes on the four odd square classes and takes the value $1 slash 2$ somewhere on each of
    the four even ones.
  + $sum_v "inv"_v cal(A)(T) = 0$ for every $T in X(QQ)$.

  #v(2mm)
  Hence $X(QQ) subset.eq { T in X(QQ_2) : "inv"_2 cal(A)(T) = 0 }$, a proper open and closed
  subset of $X(QQ_2)$, and *$X(QQ)$ is not dense in $X(QQ_2)$*.
]

Line 1 is condition (E), proved at every place in @sec-obs-status; line 2 is the verdict table
of @sec-obs-verdict, and the eight-class computation of @sec-obs-nosample proves it; line 3 is Hilbert reciprocity in $K$, regrouped by the places of $QQ$. At
$q = infinity$ there is now a one-line reason rather than an appeal to $E_d (RR)$ being connected:
at the real embedding of $K$ the discriminants of $g$ and $h$ are $-12.98$ and $-56.16$, both
negative, so *both entries are positive definite on all of $RR$* and the symbol is $+1$ for every
real point at once.

#block(fill: luma(240), inset: 8pt, radius: 3pt, width: 100%)[
  *What this buys, and what it does not.* It buys: one object instead of a family indexed by $d$;
  the twist eliminated from the statement; $Sigma(d)$ of @sec-obs-status replaced by the
  ramification of a single algebra; and the argument in the standard Brauer--Manin form, so that
...
- *(1)* The two factorisations over $K$, and $(U + d u^2) g(U slash d) = (V slash d)^2$ on $71$
  integral points of the twists --- the twist-cancellation as an exact identity.
- *(2)* $"inv"_q cal(A) = beta_q$: the twist-free form against the twisted form of
  @sec-obs-formula, $43,670$ instances over twelve twists and $q = 2,3,5,7$. No disagreement, which
  is also the check that $psi$ is normalised the way @sec-obs-formula normalises it.
- *(3)* $"inv"_q cal(A) equiv 0$ for $q = 3, 5, 7, 11, 13, 47, 61$ and at $infinity$, on $2$--$3$
  thousand sampled points of $X(QQ_q)$ each. The primes $47$ and $61$ are there because they occur
  as $q divides d$ in the table of @sec-obs-DE.
- *(4)* $"inv"_2 cal(A)$ by square class of the point: zero on all of $[1], [3], [5], [7]$ and
  non-zero on all of $[2], [6], [10], [14]$ --- @sec-obs-verdict's table, recomputed on $X$ with no
  twist anywhere in the computation.
- *(5)* $sum_v "inv"_v cal(A)(T) = 0$ on $164$ rational points of $X$, built from multiples of the
  generators of ten twist pairs.
- *(6)* $"inv"_2 cal(A)$ is constant on the discs $x_0 + 2^8 ZZ_2$, $t_0 + 2^8 ZZ_2$ around $40$
  points where it equals $1 slash 2$ --- $1960$ perturbations, all agreeing. This is what makes
  "$X(QQ)$ misses an *open* set" literal rather than formal.
